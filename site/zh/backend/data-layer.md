...
`ISqlSugarClient` 以 `SqlSugarScope` 形态注册为单例。这是 SqlSugar 官方推荐的线程安全形态，内部按线程建 Client。构造时挂上全局过滤器、审计 AOP、SQL 诊断日志。默认只有主库一条连接，ConfigId 固定为 `SmartAdmin`。

开发期想逐条看 ORM 到底生成了什么，另有一套默认关着的控制台日志：[SQL 控制台日志](./sql-log.md)。

```csharp
// 主库（零配置即这条路径）
// ConfigId = "SmartAdmin"，钩子：软删 + 数据范围 + 审计 AOP + 失败/慢 SQL 日志
return new SqlSugarScope(mainConfig, client => { /* 挂过滤器 + AOP + 日志 */ });
```

### 多数据库（多 ConfigId）

主库仍是一条连接。要再挂日志库、遗留库，写 `SmartAdmin:AdditionalDatabases`，用 `db.AsTenant().GetConnection("Audit")` 访问。`IRepository<T>` 永远打主库。
...
机器号有两个来源。不配 `SmartAdmin:Id:WorkerId`，内核启动时用文件独占锁在本机抢一个空闲槽位：依次打开 `worker-00.lock` 到 `worker-63.lock`，第一个锁得住的就是本进程的号，句柄一直持有到进程退出。配了就固定用配置值，不抢锁：
...
抢锁堵的是同机多进程这个口子。IIS 应用池重叠回收时新旧两个 w3wp 并存，Web 园、同机部署两份也是同一回事。它们读的是同一份配置，拿到的机器号完全相同，同一毫秒各发一个号就撞主键。线上「高并发同一时刻 Id 重复」几乎都从这里来，算法本身没有错。抢锁之后旧进程占着 0，新进程锁不上就落到 1，同号在结构上不可能发生。锁目录默认是机器级路径（Windows 为 `%ProgramData%\SmartAdmin\workerid`，其它系统为 `/tmp/smartadmin/workerid`），只有默认目录写不了时才用 `SmartAdmin:Id:WorkerIdLockDir` 改。目录不可用、或 64 个槽位全被占满，内核拒绝启动，不发可能重复的号。

::: danger 跨机器、跨容器每实例必须不同
文件锁只在同一台机器、同一个锁目录里互斥。**跨机器或跨容器水平扩展时，必须为每个实例显式配置不同的 `WorkerId`**，否则不同实例同毫秒发号会撞主键。两台机器的 `WorkerId` 一样，ID 里代表机器号的那 6 bit 就完全相同。只要同一毫秒里序列号也凑巧从头对齐，拼出来的 64 位数字就会一模一样。这是数据损坏级的问题，且默认静默发生。

内核给了两道防线。选了 Redis 缓存（明显的多实例意图）却没显式设置 `WorkerId` 时，启动即抛，把一个静默的主键冲突换成一条可读的启动错误。单实例请显式配 `0` 以示知情。k8s 场景可用 StatefulSet 的 Pod 序号注入。第二道在数据库：每个实例启动时按自己的机器号在 `sys_worker_lease` 表拿租约并周期续租，同库另有一个活着的实例占着同一个号，后来者启动即抛。
:::

判据落在「活着」这两个字上。节点名分两截，`@` 前是 `{机器名}#{机器号}`，`@` 后是每次启动都变的实例 token。续租和释放拿完整节点名做条件写，旧进程回魂也续不动新主的租约。拦不拦只看前半截，加上前任进程还在不在。同机同号而前任 pid 已经不在，下次启动直接接管，不用等租约到期。进程被硬杀时走不到释放那一步，停止调试、关掉控制台窗口、任务管理器都算。留下的那行拦不住下一次启动，容器重启后 pid 落回同一个号也一样。真会拦的只有两种：同机另一个进程还活着且占着同号，或者另一台机器占着同号而两边连同一个库。报错会点名对方的节点名、pid、剩余秒数，以及确认对方下线后清掉残留租约的 SQL。
