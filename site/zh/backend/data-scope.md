...
生效范围通过 `IDataScopeContext` 在请求内传递。`Current` 永远不是 null，认证请求必须在查询之前显式解析、写入，这一步由授权管道保证。没显式设置的时候怎么办，见下一节。
...
认证请求没绑上范围的时候，回退是**空范围**（机构集为空、附加「仅本人」），也就是只看得见自己创建的行，不是「不受限」。

正常路径上范围总会在授权阶段绑好，这个回退只在异常路径上兜底：兜底方向选的是让漏绑的端点表现为「查不到数据」，而不是「看到全部机构的数据」——前者有人会立刻报障，后者悄无声息，危险得多。

三类调用不受这条约束：没有 `HttpContext` 的调用（后台任务、启动期、种子，都是可信上下文）、匿名请求、超管。

只挂 `[Authorize]`、既没有 `[RolePermission]` 也没有 `[ActiveSession]` 的消费方端点，靠的是一层**全局绑定**：一个全局授权过滤器把用户的真实范围绑上去，查自己的 `DataEntity` 业务表才不会看到全部机构的数据。它是幂等的，与那两个特性叠挂时不会重复解析，匿名请求直接跳过。
...
配成 `true` 就切回未绑定即不受限这条回退路径。留这个开关是为了让升级有退路，不是给你长期开着的。

## 系统表：过滤器管不着的那一半

全局过滤器只认 `IOrgScoped`，而内核自己的 `Sys*` 表一律继承 `BaseEntity`，过滤器对它们一行都不生效。所以「谁能看见哪些日志、哪些会话」得各服务自己判。

这些表对非超管只显示范围内用户产生的行：

| 表 | 非超管看到的 |
| --- | --- |
| 操作日志、登录日志、异常日志 | 只列范围内用户产生的记录 |
| 在线会话 | 只列范围内用户的会话；强制下线只能踢范围内用户，且非超管不能踢超管 |
| 通知管理列表 | 只列范围内用户发布的通知 |

会话不存在时强退幂等返回成功（管理员点的那一行可能刚好已经过期）；会话存在但不在操作者的数据范围内，或者操作者不是超管却要踢一个超管，这两种拒绝一律抛 `42024`，响应不暴露具体是哪一种越权。会话 id 是高熵随机串，猜不出来，所以「不存在则成功、越界则报错」这点行为差异本身构不成可枚举的探测面。

判断收口在 `IDataScopeGuard`（机构越界断言、用户查询的范围条件、范围内用户 Id 集三件事），用户、机构、职位、文件、日志、会话六个服务共用同一份判断。收口在一处的价值就在于日志与会话也吃得到这层约束：没有它，被授了日志查看权限的子管理员能读到全公司的操作记录（含入参 JSON），还能强退任何人。

范围内用户 Id 集**含软删用户**。日志与会话是历史事实，离职的人做过什么，不该随着账号被软删而消失。

## 扩展点

`IDataScopeProvider` 是替换点。默认实现聚合的是 `sys_role_data_scope`。典型的替换是换一个隔离维度，比如按租户隔离。`ResolveAsync` 和内部的 `ComputeAsync` 都是 `virtual`，只覆写其中一步也行。按内核的可替换性约定，消费方在 `AddSmartAdmin()` 之前注册自己的 `IDataScopeProvider` 就能接管，不用 fork。
