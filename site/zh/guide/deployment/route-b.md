...
反向代理托管静态产物，把 `/api` 和实时通知的 `/hub` 转给后端。浏览器只看到一个源，所以**同样不需要 CORS**。nginx 和 Caddy 两份配置等价，选一份抄走就行。
...
server {
    ...
    root /var/www/smart;          # web/dist 的内容
    index index.html;
    ...
    # SignalR 实时通知 Hub:WebSocket 升级必须显式转发 Upgrade/Connection,
    # 漏掉这段,实时通知会静默退回 30 秒轮询。
    location /hub/ {
...
admin.example.com {
    ...
    # SignalR 实时通知 Hub:reverse_proxy 自动处理 WebSocket 升级
    handle /hub/* {
        reverse_proxy 127.0.0.1:5000
    }
