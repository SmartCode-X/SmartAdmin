...
主库仍是 `SmartAdmin:Database` 那一条。要再挂日志库、遗留库、只读副本，写 `SmartAdmin:AdditionalDatabases`，用 `db.AsTenant().GetConnection("名字")` 访问。
...
主库形状不变。副库是数组，可以挂多条。Worker 宿主走 `AddSmartAdminWorker` 时，同一套 `SmartAdmin` 节同样生效。
...
| 字段 | 必填 | 默认 | 说明 |
| --- | --- | --- | --- |
| `ConfigId` | 是 | — | 连接名。全局唯一（大小写不敏感）；**不能**叫 `SmartAdmin`（主库保留，`smartadmin` 也会拒） |
...
下列情况在 `AddSmartAdmin` 时直接抛 `InvalidOperationException`：

- 数组元素为 null  
- `ConfigId` 空白  
- 占用保留名 `SmartAdmin`（忽略大小写）  
...
在 `AddSmartAdmin` **之前**整包 `TryAddSingleton<ISqlSugarClient>` 替换后，内置装配被跳过，`AdditionalDatabases` **不会**再生效，多库由你自己管。
