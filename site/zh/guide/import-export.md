...
装 `SmartAdmin.Excel`、写一份档案、在资源控制器上挂六个端点，业务表就能走 xlsx 导入导出。不装这个包，凡要读写 xlsx 的调用一律返回 `46001`，部署体积也不会多一个字节。

实体和 CRUD 还没有的话，先走[加一个业务模块](/zh/guide/business-module)。Agent 施工用的逐步清单在仓库 `skills/wire-import-export.md`。

## 先装可选包，再调内核

`SmartAdmin` 元包不引用 Excel。默认 codec 是 `MissingExcelProvider`：读写、生成模板都会抛 `ErrorCode.ExcelProviderMissing`（码 `46001`）。这是可选性的定义。

```bash
dotnet add package SmartAdmin.Excel
```

注册顺序和 Redis / 外部登录一样：**先** `AddSmartAdminExcel()`，**再** `AddSmartAdmin()`。内核用 `TryAdd` 挂默认实现，先到者胜：

```csharp
using SmartAdmin.Excel;

builder.Services.AddSmartAdminExcel(); // 必须在前面
...
可选配置节 `SmartAdmin:Excel`：`MaxImportRows`（默认 5000）、`MaxExportRows`（默认 50000）、`MaxImportFileSizeMb`（默认 10，**不**与上传头像共用上限）。
...
| 文件 | 作用 |
|---|---|
| `backend/tests/SmartAdmin.TestHost/SampleDoc.cs` | 机构隔离业务表 |
...
`[RolePermission]` 端点要有菜单按钮节点，Permission 写成 `METHOD:/路由模板`，与控制器一字不差。导入、导出是页面的特有操作，按钮 Id 的个位落在 5–9，1–4 固定留给查询、新增、更新、删除。可以直接对照内核：用户管理页 230 的导入是 237、导出是 238，操作日志页 520 的导出是 526。消费方预置菜单 Id 从 `SmartSeedIds.ConsumerMin`（1000）起。模板下载走 `[ActiveSession]`，没有按钮。

前端的导入向导 `ImportWizard` 与导出选列弹窗 `ExportColumnsModal` 都从 `smart-admin-web` 导入。`gen:api` 后按钮用 `v-auth` 挂真实权限码。

## 库为什么是 MiniExcel + OpenXml

选型是 MiniExcel + OpenXml，不是 Magicodes.IE：后者依赖闭包带原生库，校验够不到运行期字典表，错误信息还是英文串，和「错误只带数字码、文案在前端」冲突。MiniExcel 做读写、DocumentFormat.OpenXml **只**生成带下拉的模板，二者都是纯托管，现有 ASP.NET 镜像不用改。
