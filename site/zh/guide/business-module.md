...
在 SmartAdmin 之上加一张业务表、一套接口，不改内核一行代码。这页从选实体基类一路走到接口能被授权、能被前端调通。

::: tip 两条路线，只差「代码放哪」
- **路线 A**：直接改本仓库，在内核内加，新代码进 `SmartAdmin.Services` / `SmartAdmin.AspNetCore`。
- **路线 B**：你的项目装了 `SmartAdmin` NuGet 包，在自己的业务程序集里加，靠 `options.ApplicationAssemblies` 挂载，内核源码一行不碰。
...
这页不编「商品」示例，而是带你读两处真实存在、都有测试跑着的代码：

- `backend/src/SmartAdmin.Services/Dict/`：内核内置字典模块，普通表（不按机构隔离）的范本。
- `backend/tests/SmartAdmin.TestHost/`：集成测试用的消费方宿主，里面的 `SampleDoc` 是一个货真价实的机构隔离业务模块，走的正是路线 B。
...
- **不需要**（全局共享，比如字典、配置）→ 继承 `BaseEntity`。字典类型实体 `SysDictType`（`backend/src/SmartAdmin.SqlSugar/Entities/`）就是这样。
- **需要**（不同机构的用户只看得到、改得动自己机构的数据）→ 继承 `DataEntity`。它自带 `CreateOrgId` 锚点，查询会被全局过滤器按当前用户的数据范围自动裁剪。

`backend/tests/SmartAdmin.TestHost/SampleDoc.cs` 是后者的真实范本：
...
把 `SampleDoc` 换成你自己的实体名和字段，放进消费方项目自己的程序集就行（路线 A 才放进 `SmartAdmin.Services`）。

## 服务：读走过滤器，写先校验可见性

契约和实现的完整范本是 `backend/tests/SmartAdmin.TestHost/SampleDocService.cs`。三个读写要点决定了它是否「按机构隔离得住」：
...
每个动作挂 `[RolePermission]`，权限码就是规范化后的路由本身，代码里不写任何权限字符串，完整代码在 `backend/tests/SmartAdmin.TestHost/SampleDocController.cs`：
...
消费方不改内核，在自己的 `Program.cs` 里做两件事（完整代码见 `backend/tests/SmartAdmin.TestHost/Program.cs`）：
...
// 自己的服务用 TryAdd 注册(未被内核占用的接口,AddSmartAdmin 之前/之后都行)
...
必须用 `TryAdd`，不是 `Add`。这样消费方才能在 `AddSmartAdmin()` **之前**注册同接口的自定义实现来覆盖默认行为。这是整个内核「可替换」设计的根规则。内置服务（比如 `Dict`）在 `ServicesSetup.cs` 里也是这么注册的。你自己的服务如果没人跟你抢接口，`TryAdd` 和 `Add` 效果一样。统一写 `TryAdd`，省得日后被覆盖时踩坑。

::: warning 忘了 `ApplicationAssemblies.Add(...)` 就静默 404
内核不会自动扫描发现你的模块，也没有 `ScanApplicationAssemblies` 之类的开关可开。漏了这一行，`SampleDoc` 不会建表，`SampleDocController` 也不会注册路由。直接 404，没有任何兜底开关或提示。
:::

## 错误码（可选）

要精确区分「不存在」和其他失败，内核内置模块的做法是往 `Core/ErrorCode.cs` 枚举里加数字码，**只加码，不写文案**。字典模块的 `DictTypeNotFound = 43001`、`DictTypeCodeExists = 43002` 就是例子，文案由前端按信封里的 `msgKey` 翻译。消费方另建自己的错误码枚举，码从 `60000` 起，每个成员标 `[MsgKey]`，放在 `ApplicationAssemblies` 登记过的程序集里就会被自动登记，抛的时候强转成 `ErrorCode`，写法见[错误码](/zh/backend/error-codes)。结果本来就能用返回值表达的，也可以像 `SampleDocService` 那样直接返回（`false` 表示不存在/无权）。

## 种子数据（可选）

需要出厂默认数据时，实现泛型 `ISeedData<T>`，给每行一个固定 `Id` 保幂等。注意别直接实现非泛型 `ISeedData`：它只是 DI 收集用的空标记，直接实现虽然能编译，但启动时反推不出实体类型会崩。内核自己的写法在 `Seed/DictSeed.cs`，消费方范例是 `backend/tests/SmartAdmin.TestHost/SampleWidgetSeed.cs`：
...
固定 `Id` 必须落在消费方保留区间 `Id ≥ 1000`（常量 `SmartSeedIds.ConsumerMin`），上限不是写死的数字，而是启动时刻算出的雪花地板。别沿用「随手挑个小整数」的老习惯。你和内核会往同一批表里播种，比如 `sys_menu`、`sys_config`。今天不撞，不代表升级内核包后也不撞。到那时你库里已经有那行，退不回去了：
...
菜单种子的编号照内核那套来，只是整体挪到 1000 以上。每个目录占一个百位段，比如 1000、1100。页面取整十，按钮看个位：**1 查询、2 新增、3 更新、4 删除**，5–9 放本页特有操作，缺的标准按钮空着位。其他种子没有层级，从 1000 起顺序往上编就行。

::: warning 种子 Id 撞号或越界，启动即拒
撞号的固定 Id 一旦没被拦下，后果是悄无声息坏掉：幂等判存会把后来那行当「已存在」跳过，菜单树因此无声缺一块；种子升级要是还开着 `SyncOnUpgrade`，会把别人那行直接覆盖掉。`DatabaseInitializer` 因此在启动时逐实体登记所有种子声领的固定 Id，内核和消费方的都登记，一旦发现越界或同实体重复就当场抛异常，应用直接起不来，并告诉你该换哪段号。`SeedIdRangeTests` 里有对应的契约测试，CI 会在任何宿主启动前先变红。它既盖住「复制行忘改 Id」的自撞，也盖住跨种子撞号。
:::
...
1. 进**菜单管理**，建菜单节点：`Type=菜单`、`Path` 填前端路由地址（如 `/sample/doc`）、`Component` 填页面 key（如 `sample/doc/index`，即 `views/` 之后去掉 `.vue` 的路径）、`所属应用`选一个顶级目录。
2. 建 `Type=按钮` 节点，每页固定四颗：查询、新增、更新、删除。`Permission` 从路由下拉里多选，「查询」把列表与详情两条路由都挂上，多条以 `;` 连接。前端 `v-auth` 仍按单条路由码显隐按钮。
3. 进**角色管理**，给角色勾选该菜单/按钮，该角色下的用户即获得对应路由权限，授权变更即时生效（内核会失效对应缓存）。
4. 超管开发期不用配权，自动放行全部路由。

接口没有页面（只给移动端、PDA 或第三方系统调）时，不用建假页面：建一个目录当权限组，`Path` 和 `Component` 留空，把 `Type=按钮` 节点直接挂在它下面。角色授权界面会把这些按钮渲染成该目录组里「接口权限（无页面）」一行，前端不生成路由、侧栏不显示。内核的 `GET:/api/v1/ping` 就是这么挂的。

想让菜单出厂就预置（而不是每套环境手点），用种子 `DefaultMenuSeed` 那套写法批量播 `SysMenu` 行（菜单节点、按钮节点都是 `SysMenu`），取号照上一节的编号法。给内置模块**改**已有种子行（比如同 Id 补一个字段）时要记得 bump `SysSchemaVersion.Current`。老库只有版本号变了才会经 `SyncOnUpgrade` 回填（见 `SqlSugar/Entities/SysSchemaVersion.cs` 注释）；纯新增行不需要 bump。
...
装 `SmartAdmin.Testing` 包（`dotnet new smart-app` 的 `Tests/` 已经装好），`AppFactory : AdminAppFactory<Program>` 就是你的宿主：一次性库、固定超管密码与 JWT 密钥、环境钉 Development。用它写 HTTP 级回归（蓝本 `backend/tests/SmartAdmin.Tests/ModulePortalTests.cs`）：造用户、授菜单 → 带 token 调端点 → 用 `ReadEnvelope()` 断言信封。SQLite/MySQL 两条腿都要绿（`TestDb` 按 `SMART_TEST_DBTYPE` 等环境变量派生隔离库，与内核自己的测试同一套）：
