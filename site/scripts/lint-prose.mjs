...
//   5. 每页破折号 —— 超过 2 个
//
// 判据是「紧贴中文」而不是「出现半角」：Result<T>、AddSmartAdmin()、SmartAdmin:Database
...
// `**验证参数**(SmartAdminSetup.cs)`、`**两端都要配**:` 里的半角，读者看得见，规则看不见。
...
// 正文第一句。
// 引用块和列表【不豁免】：components/smart-icon.md 的开篇正文就写在 `>` 里，
...
if (process.argv[2] === '--selftest') {
  ...
  eq(has(R('halfwidth-paren'), '调用 AddSmartAdmin() 之后'), false, '真·空括号（裸函数调用）→ 放过')
  ...
  eq(has(R('halfwidth-between-code'), '装配次序（见 `SmartAdminSetup.cs`）:'), true, '全角括号后的行尾半角冒号 → 拦')
  eq(has(R('halfwidth-between-code'), '装配次序（见 `SmartAdminSetup.cs`）：'), false, '同上但用全角 → 放过')
  ...
  eq(has(R('halfwidth-paren'), '**验证参数**(`SmartAdminSetup.cs`)'), true, '粗体中文 + 半角括号 → 拦')
  eq(has(R('halfwidth-comma'), '**错误是数字，从不下发文案**,后端只给码'), true, '粗体中文 + 半角逗号 → 拦')
  eq(has(R('halfwidth-colon'), '**两端都要配**:'), true, '粗体中文 + 半角冒号 → 拦')
  eq(has(R('halfwidth-paren'), '**验证参数**（`SmartAdminSetup.cs`）'), false, '粗体中文 + 全角括号 → 放过')
...
for (const file of files.sort()) {
  ...
  if (first) {
    // 自称词全列封死：只封一部分，换个自称词就能把同一个槽位原样写回。
