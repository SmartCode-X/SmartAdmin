...
The main database is still `SmartAdmin:Database`. To attach a log store, legacy database, or read replica, add `SmartAdmin:AdditionalDatabases` and call `db.AsTenant().GetConnection("name")`.
...
The main-database shape is unchanged. Secondaries are an array. Worker hosts using `AddSmartAdminWorker` bind the same `SmartAdmin` section.

### Add from code

```csharp
builder.Services.AddSmartAdmin(builder.Configuration, opt =>
{
...
| --- | --- | --- | --- |
| `ConfigId` | yes | — | Connection name. Globally unique (case-insensitive); must **not** be `SmartAdmin` (reserved; `smartadmin` is rejected too) |
...
These throw `InvalidOperationException` during `AddSmartAdmin`:

- null array element  
- blank `ConfigId`  
- reserved name `SmartAdmin` (case-insensitive)  
...
If you `TryAddSingleton<ISqlSugarClient>` **before** `AddSmartAdmin`, the built-in registration is skipped and `AdditionalDatabases` has **no** effect — you own multi-DB yourself.
