...
Add a business table and a set of endpoints on top of SmartAdmin without changing a single line of kernel code. This page walks from choosing an entity base class all the way to an endpoint that can be authorized and called from the frontend.

::: tip Two routes, differing only in "where the code lives"
- **Route A** — modify this repo directly, adding inside the kernel; the new code goes into `SmartAdmin.Services` / `SmartAdmin.AspNetCore`.
- **Route B** — your project has installed the `SmartAdmin` NuGet package, and you add it in your own business assembly, mounted via `options.ApplicationAssemblies`, without touching a line of kernel source.
...
Rather than inventing a "product" example, this page walks you through two places that genuinely exist and have tests running against them:

- `backend/src/SmartAdmin.Services/Dict/` — the kernel's built-in dictionary module, a template for a plain table (not org-isolated).
- `backend/tests/SmartAdmin.TestHost/` — the consumer host used for integration tests, where `SampleDoc` is a genuine org-isolated business module, following exactly Route B.
...
- **No** (globally shared, e.g. dictionaries, config) → inherit `BaseEntity`. The dictionary-type entity `SysDictType` (`backend/src/SmartAdmin.SqlSugar/Entities/`) is like this.
...
`backend/tests/SmartAdmin.TestHost/SampleDoc.cs` is a real example of the latter:
...
Swap `SampleDoc` for your own entity name and fields, and drop it into your consumer project's own assembly (only Route A puts it in `SmartAdmin.Services`).

## Service: reads go through the filter, writes check visibility first

The full template for the contract and implementation is `backend/tests/SmartAdmin.TestHost/SampleDocService.cs`. Three read/write points decide whether it "holds up" as org-isolated:
...
`backend/tests/SmartAdmin.TestHost/SampleDocController.cs` — every action carries `[RolePermission]`, the permission code is the normalized route itself, and no permission string is written anywhere in code:
...
Without touching the kernel, a consumer does two things in their own `Program.cs` — full template at `backend/tests/SmartAdmin.TestHost/Program.cs`:
...
// Register your own services with TryAdd (for interfaces the kernel hasn't claimed, before or after AddSmartAdmin is fine)
...
You must use `TryAdd`, not `Add`. That's what lets a consumer register a custom implementation of the same interface *before* `AddSmartAdmin()` to override the default behavior — the root rule behind the kernel's entire "replaceable" design, and how built-in services (like `Dict`) are registered in `ServicesSetup.cs` too. For your own service, if nobody's competing for the interface, `TryAdd` and `Add` behave the same; write `TryAdd` uniformly and you won't get bitten when something does come along to override it later.

::: warning Forget `ApplicationAssemblies.Add(...)` and it's a silent 404
The kernel **does not auto-discover** your module, and there's no `ScanApplicationAssemblies`-style switch to flip. Miss this one line and `SampleDoc` gets no table, `SampleDocController` registers no routes — a flat 404, with no fallback switch or hint.
:::

## Error codes (optional)

To distinguish "not found" from other failures precisely, the kernel's built-in modules add a numeric code to the `Core/ErrorCode.cs` enum — **codes only, no message text** — as with the dictionary module's `DictTypeNotFound = 43001` and `DictTypeCodeExists = 43002`; the frontend translates the text from the envelope's `msgKey`. A consumer defines its own error-code enum starting at `60000`, marks each member with `[MsgKey]`, and keeps it in an assembly registered in `ApplicationAssemblies`, which registers it automatically; it is thrown by casting to `ErrorCode`, as shown in [Error Codes](/backend/error-codes). When a return value already says enough, return it directly the way `SampleDocService` does (`false` meaning not found / no access).

## Seed data (optional)

When you need out-of-the-box default data, implement the **generic** `ISeedData<T>` (the non-generic `ISeedData` is just an empty marker for DI collection — implementing it directly compiles, but crashes at startup when the entity type can't be inferred), and give each row a fixed `Id` for idempotency. Blueprint: `Seed/DictSeed.cs`; consumer example: `backend/tests/SmartAdmin.TestHost/SampleWidgetSeed.cs`:
...
public sealed class SampleWidgetSeed : ISeedData<SampleWidget>
{
    ...
    [
        new() { Id = SmartSeedIds.ConsumerMin,     Name = "widget-a" },
        new() { Id = SmartSeedIds.ConsumerMin + 1, Name = "widget-b" },
    ];
}
```
...
Number menu seeds the way the kernel does, just shifted above 1000. Each directory gets a block of a hundred, such as 1000 or 1100. Pages take the round tens, and a button's last digit says what it is: **1 Query, 2 Create, 3 Update, 4 Delete**, with 5–9 for the page's own actions; a page missing a standard button leaves that slot empty. Other seeds have no hierarchy, so numbering them upward from 1000 is enough.

::: warning A colliding or out-of-range seed Id refuses startup
Left uncaught, a colliding fixed Id breaks **silently**: the idempotent existence check skips the later row as "already there" (a piece quietly missing from the menu tree), and a seed with `SyncOnUpgrade` on would even overwrite the other row on upgrade. `DatabaseInitializer` therefore registers, per entity at startup, every fixed Id claimed by any seed (kernel + consumer), and the moment it finds one out of range or duplicated within an entity it **throws on the spot and the app won't start**, telling you which range to move to; `SeedIdRangeTests` carries the matching contract test, so CI goes red before any host even boots. This covers both the self-collision of "copied a row and forgot to change the Id" and cross-seed collisions.
:::
...
1. Go to **Menu Management** and create a menu node: `Type=Menu`, `Path` set to the frontend route (e.g. `/sample/doc`), `Component` set to the page key (e.g. `sample/doc/index`, the path after `views/` minus `.vue`), and `App` set to a top-level directory.
2. Create `Type=Button` nodes, the same four on every page: Query, Create, Update, Delete. Pick `Permission` from the route dropdown (multiple allowed) — Query carries both the list and the detail route, joined by `;`. The frontend's `v-auth` still shows or hides a button by a single route code.
...
When an endpoint has no page (it is only called by a mobile app, a handheld scanner or a third-party system), don't build a fake page for it: create a directory as the permission group, leave `Path` and `Component` empty, and hang the `Type=Button` nodes directly under it. The role-grant screen renders those buttons as an "API only (no page)" row inside that directory's group; the frontend builds no route and the sidebar shows nothing. The kernel's `GET:/api/v1/ping` is wired this way.
...
Install the `SmartAdmin.Testing` package (the `Tests/` project `dotnet new smart-app` scaffolds already has it) and `AppFactory : AdminAppFactory<Program>` is your host: a throwaway database, a fixed super-admin password and JWT key, environment pinned to Development. Write HTTP-level regression tests on it (blueprint: `backend/tests/SmartAdmin.Tests/ModulePortalTests.cs`): create a user and grant a menu → call the endpoint with a token → assert the envelope with `ReadEnvelope()`. Both the SQLite and MySQL legs must be green (`TestDb` derives an isolated database from `SMART_TEST_DBTYPE` and friends, the same one the kernel's own tests use):

```bash
dotnet test backend/SmartAdmin.slnx -- --filter-class "*SampleDocScopeTests*"
```
