...
Install `SmartAdmin.Excel`, write a profile, and hang six endpoints on the resource controller — that is enough for xlsx import/export on a business table. Without the package, every call that reads or writes xlsx returns `46001`, and publish size does not grow by a single byte.
...
## Install the optional package, then the kernel

The `SmartAdmin` meta-package does **not** reference Excel. The default codecs are `MissingExcelProvider`: read, write, and template generation all throw `ErrorCode.ExcelProviderMissing` (`46001`). That is what “optional” means.

```bash
dotnet add package SmartAdmin.Excel
```

Registration order matches Redis and external login: call `AddSmartAdminExcel()` **before** `AddSmartAdmin()`. The kernel registers defaults with `TryAdd` (first registration wins):

```csharp
using SmartAdmin.Excel;

builder.Services.AddSmartAdminExcel(); // must come first
...
Optional config under `SmartAdmin:Excel`: `MaxImportRows` (default 5000), `MaxExportRows` (default 50000), `MaxImportFileSizeMb` (default 10 — **not** shared with avatar upload limits).
...
| File | Role |
|---|---|
| `backend/tests/SmartAdmin.TestHost/SampleDoc.cs` | Org-scoped business table |
...
Every `[RolePermission]` endpoint needs a menu button whose `Permission` is `METHOD:/route-template`, character-identical to the controller. Import and export are page-specific actions, so their button Ids end in 5–9; the last digits 1–4 are reserved for Query, Create, Update and Delete. The kernel is a ready reference: on the user page (230) import is 237 and export is 238, and the op-log page (520) exports through 526. Consumer menu seeds start at `SmartSeedIds.ConsumerMin` (1000). Template download is `[ActiveSession]`-only and has no button.

The import wizard `ImportWizard` and the export column picker `ExportColumnsModal` both come from `smart-admin-web`. After `gen:api`, gate buttons with `v-auth` and the real permission codes.

## Why MiniExcel + OpenXml

The pick is MiniExcel + OpenXml, not Magicodes.IE: the latter carries native assets in its dependency closure, its compile-time attributes can't see runtime dict tables, and its error strings are English — which fights "numeric `ErrorCode` only, copy on the frontend." MiniExcel handles read/write; DocumentFormat.OpenXml is used **only** to build templates with dropdowns. Both are pure managed code — the stock ASP.NET image needs no change.
