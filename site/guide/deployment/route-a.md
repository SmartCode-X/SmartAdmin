...
2. Add two lines of **plain ASP.NET Core** code to your own `Program.cs` (the kernel doesn't manage frontend hosting, so there's no `MapSmartAdminSpa`-style wrapper):

```csharp
var app = builder.Build();

app.UseStaticFiles();               // serve the frontend build under wwwroot
app.MapSmartAdmin();                // API endpoints (the fallback always matches last, whatever the order)
...
```json
{ "SmartAdmin": { "Upload": { "RootPath": "./storage/upload" } } }
```
