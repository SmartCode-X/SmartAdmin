...
- **Response side** — the backend's `PagedList<T>` (`{ current, size, total, items }`) becomes `{ items, total }`, exactly the shape SmartTable's `fetcher` contract expects.
...
The root of the convention: **the backend never sends localized text.** Every business error is a numeric `ErrorCode` (`backend/src/SmartAdmin.Core/ErrorCode.cs`), and each enum member is tagged with a `[MsgKey("...")]` dot-path:
...
```ts
// kernel package locales/zh-CN.ts
error: {
...
`translateError` in `utils/error.ts` is where the two sides actually meet; an app imports it from `smart-admin-web`:
...
Timeouts and deliberate cancellations are recognized first by `abortMsgKey` and get their own text. Everything else resolves by `msgKey`, with `args` passed along to fill placeholders; only without a `msgKey` does it fall back to the numeric-code whitelist `CODE_MSG_KEY`, which covers the kernel's own codes only. `i18n.global.te(key)` just checks whether the key exists in the current locale — a purely local probe, no network call. So the full path for a `FileTooLarge` (44002) is:
...
Suppose the kernel gains a new error code — `[MsgKey("error.file.duplicateHash")] FileDuplicateHash = 44007` — but nobody adds `error.file.duplicateHash` to the kernel's `zh-CN.ts` / `en-US.ts`. Here's what happens: `i18n.global.te(...)` returns `false`, and `translateError` falls through to `err.message`. But `err.message` defaults to the msgKey itself (`Result.Message = message ?? code.GetMsgKey()`), so what actually lands in the UI is the raw string `error.file.duplicateHash`, verbatim. **Adding a backend `ErrorCode` and adding the matching frontend `error.xxx.yyy` key are two halves of one change** — and this isn't only a runtime embarrassment: `ErrorCodeLocaleConsistencyTests` checks that every `[MsgKey]`'s leaf segment exists in both language packs, so a missing entry turns a **backend** test red — while the person debugging it is usually looking on the frontend. An app's own error codes are outside that test's reach: their text goes into `src/locales/ext/<locale>/error.ts`, in both languages.
:::

## Regenerating the contract

`schema.d.ts` isn't hand-written — it's generated from the `/openapi/v1.json` of a **running** backend instance. After changing a backend endpoint or DTO, regenerate it from the matching `web/`:

```bash
npm run gen:api
```

The key word is "running": `gen:api` uses openapi-typescript's live fetch, not an offline scan of the code. Before you run it the backend must be up (`dotnet run --project backend/samples/MinimalHost`, or `dev-start.bat` to start both halves at once), and `http://localhost:5100/openapi/v1.json` must be reachable — only then does `gen:api` have a contract to fetch. With the backend not started, that endpoint isn't reachable and the command gets nothing back.
