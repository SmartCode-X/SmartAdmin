...
给已有实体接 xlsx 导入 / 导出。档案声明列与业务动作，编排走内核 `IImportRunner`，编解码走可选包 `SmartAdmin.Excel`。
...
| 样板 | 路径 | 演示什么 |
|---|---|---|
| `SampleDoc`（`DataEntity`） | `backend/tests/SmartAdmin.TestHost/SampleDoc.cs` | 机构隔离业务表 |
...
| `UserImportProfile` | `backend/src/SmartAdmin.Services/ImportExport/UserImportProfile.cs` | 完整 `IImportProfile`（字典 + 按名查外键 + 越权） |
| `UserController` 导入导出段 | `backend/src/SmartAdmin.AspNetCore/Controllers/UserController.cs` | 六个端点形状 |
| `MinimalHost` | `backend/samples/MinimalHost/Program.cs` | `AddSmartAdminExcel()` 装配顺序 |

---

## 第 0 步：装可选包（不装就全抛 46001）

`SmartAdmin` 元包**不**引用 Excel。默认 codec 是 `MissingExcelProvider`，任意读写 / 模板调用一律抛 `ErrorCode.ExcelProviderMissing`（46001）。这是可选性的定义，不是 bug。

```bash
dotnet add package SmartAdmin.Excel
```

开发内核仓内可用 `ProjectReference` 代替（照 `MinimalHost.csproj`）。

**必须在 `AddSmartAdmin()` 之前**调用 `AddSmartAdminExcel()`（`TryAdd` 先到者胜）：
...
`[RolePermission]` 端点必须有对应菜单按钮种子，否则 `PermissionCodeConsistencyTests` 反向锁变红（系统模块）。业务模块通常走后台「菜单管理」UI；要预置则自注册 `ISeedData<SysMenu>`，Id ∈ 消费方区间（`≥ SmartSeedIds.ConsumerMin`）。

### 系统模块（内核 `DefaultMenuSeed`）

规则与 `create-crud-backend.md` 相同，补进导入、导出两颗按钮：

1. 在 `DefaultMenuSeed.cs` 里找到该资源管理页的十位段（页面 Id 取整十，如用户管理 230）。
2. **导入、导出是本页特有操作，个位落在 5–9 的空位**，接在已有特有按钮之后；1–4 固定是查询、新增、更新、删除，不占。
3. 内核种子 Id ∈ `[1, 999]`（`SeedIdRangeTests`），编号规则由 `MenuSeedIdLayoutTests` 锁定。
4. `ParentId` = 该资源管理页的菜单 Id；`Sort` 接该页已有按钮末尾。
5. `Permission` = `METHOD:/路由模板`，与控制器路由一字不差。**导入的四步（预览 / 重验 / 错误报告 / 提交）是一颗按钮**，四条路由用 `PermissionCode.Join` 连起来——勾「导入」就能走完整个向导，不存在勾了预览没勾提交的半截权限。

用户资源实装（用户管理页 230，导入、导出占 7、8 号位）：

```csharp
// template 走 [ActiveSession]，无需权限节点
new SysMenu { Id = 237, ParentId = 230, Type = MenuType.Button, Title = "用户-导入",
    Permission = PermissionCode.Join([
    ...
    Sort = 7, Enabled = true },
new SysMenu { Id = 238, ParentId = 230, Type = MenuType.Button, Title = "用户-导出",
    Permission = "GET:/api/v1/sys/user/export", Sort = 8, Enabled = true },
```

消费方业务菜单种子 Id 从 `SmartSeedIds.ConsumerMin`（1000）起，**不要**占用 `[1, 999]`，编号照同一套规则。

前端按钮门控：用 `v-auth="'POST:/api/v1/sys/user/import/preview'"` 门控导入入口、`'GET:/api/v1/sys/user/export'` 门控导出入口——仍是单条路由码，与按钮挂几条无关。
...
内核 `ImportRunner` 两种输入都接受：`ToValueAsync` 失败时再查它是不是已经是合法 value。你接自己的实体时，**测试必须把 Preview 的输出喂回 Validate / Commit**，不要每次手造带 label 的行：
...
- [ ] 已 `dotnet add package SmartAdmin.Excel`（或 ProjectReference）
- [ ] `AddSmartAdminExcel()` 写在 `AddSmartAdmin()` **之前**
...
- [ ] 后端跑着时重新生成前端契约（系统模块在 `web/`、业务模块在应用根目录 `npm run gen:api`）；按钮 `v-auth` 用真实权限码

```bash
dotnet build backend/SmartAdmin.slnx -c Release
dotnet test  backend/SmartAdmin.slnx -- --filter-class "*ImportExport*"
```

前端接线（向导组件、blob 下载、multipart）见 `web/packages/admin/src/components/ImportWizard/` 与 `web/COMPONENTS.md`；业务模块从包里导入：`import { ImportWizard, unwrapDownload } from 'smart-admin-web'`。
