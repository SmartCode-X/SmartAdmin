...
消费者要「每天三点跑一次对账」时的完整路径:写一个 `IAdminJob`、注册一行、在后台建任务。**不需要写调度代码、不需要建表、不需要加端点**——内核的定时任务模块已经把这些做完了(面向读者的文档见 `site/zh/guide/scheduled-jobs.md`)。
...
internal sealed class ReconcileJobSeed : ISeedData<SysJob>
{
    ...
    [
        new SysJob
        {
            Id = SmartSeedIds.ConsumerMin,             // 消费者段 ≥ 1000
            Code = "biz-reconcile",
            Name = "每日对账",
...
- **HTTP 任务**:属性包填 `url`(必)、`method`、`headers`(JSON 对象串)、`body`、`successStatuses`。目标地址过 SSRF 围栏——默认只封云元数据段,内网放行;要收紧配 `SmartAdmin:Jobs:Http:AllowedHosts`。
- **SQL 任务**:属性包填 `sql`。总闸 `SmartAdmin:Jobs:Sql:Enabled` 默认关,**打开等于承认「能编辑任务的人 = DBA」**。

## 五、验证

```bash
dotnet test backend/SmartAdmin.slnx -- --filter-class "*Job*"
dotnet run --project backend/samples/MinimalHost     # 建任务 → 执行一次 → 看执行记录
```

写单元测试直接调 `ExecuteAsync`,不必启调度器:处理器是个普通 Scoped 服务,`JobExecutionContext` 是个可 `new` 的记录快照。要测调度行为(推格触发、misfire、重试)照抄 `backend/tests/SmartAdmin.Tests/JobEngineHost.cs` 的裸容器 + 可拨时钟成法。
