...
在 SmartAdmin 中创建一个 SqlSugar 实体类。

## 第一步：确定模式

- **系统模块（内核维护者）**：文件放 `backend/src/SmartAdmin.Services/Entities/Sys{Name}.cs`，命名空间 `SmartAdmin.Services`，表名 `sys_xxx`
...
| 基类 | 何时用 | 自带字段 |
|---|---|---|
| `BaseEntity` (命名空间 `SmartAdmin.SqlSugar`) | 无需按组织隔离的表（字典、配置、全局数据） | `Id`, `CreateTime`, `CreateUserId`, `UpdateTime`, `UpdateUserId`, `IsDelete` |
| `DataEntity` (命名空间 `SmartAdmin.SqlSugar`) | 需要按组织数据权限隔离的业务表 | 继承 BaseEntity 全部 + `CreateOrgId`（归属机构锚点） |
...
5. **using**：`using SqlSugar;` + `using SmartAdmin.SqlSugar;`

### 参考模板

```csharp
// 文件: backend/src/SmartAdmin.Services/Entities/SysPosition.cs
using SqlSugar;
...
字典类型 / 字典项 / 配置项的删除接口按 **`Id < 1000` 判定为内核种子并拒删**（`ErrorCode.SeedDataProtected`）。判定只看 Id，不看 code，所以新增内核种子会自动受保护——代价是消费者写种子时必须避开这个区间：
