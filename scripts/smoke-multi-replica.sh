#!/usr/bin/env bash
# Dual-replica smoke test: verifies the guarantees (force-logout, lockout threshold, cluster-wide
# rate limit, distinct WorkerId, real client IP, single scheduler leader with failover) that
# **only surface when two replicas are online at the same time**. Running a single replica ten
# thousand times would never reveal these issues, so this script is the only real evidence for them.
#
...
ADMIN_PASSWORD="${SMART_ADMIN_PASSWORD:-Smart@123456}"
FAILURES=0
...
for i in $(seq 1 120); do
  # 正文是 JSON 了(状态码不变),挑 status 字段而不是整段比对
  if curl -s "$BASE/health/ready" 2>/dev/null | grep -q '"status":"Healthy"'; then
    STREAK=$((STREAK + 1))
...
# 20/min, so two replicas would allow 40/min), silently halving the brute-force rate-limit baseline.
...
# it is NOT what gates the claim CAS. Measured 2026-07-27:
...
if [ "$NODE_COUNT" -lt 2 ]; then
  ...
  # The job target must NOT be loopback: Jobs:Http:BlockedCidrs blocks 127.0.0.0/8 and ::1/128 by
  # default, so a job pointed at the replica's own port is rejected with 47009 before it ever runs.
  # Use the sibling replica's compose service name -- it resolves to a 172.x container address, which
  # the fence allows, and it still proves the HTTP handler actually fired.
  JOB_CODE="smoke-tick-$RANDOM"
  JOB_BODY="{\"code\":\"$JOB_CODE\",\"name\":\"smoke tick\",\"handlerKind\":2,\"handlerName\":\"\",\"triggerKind\":2,\"intervalSeconds\":5,\"properties\":{\"url\":\"http://app:8080/health\"}}"
...
print(next((n['nodeName'] for n in nodes if n['isLeader']), ''))")
    LEADER_SVC=$(docker compose ps --format '{{.Service}} {{.Name}}' 2>/dev/null | while read -r svc name; do
      case "$svc" in app|app2)
        wid=$(docker exec "$name" printenv SmartAdmin__Id__WorkerId 2>/dev/null)
