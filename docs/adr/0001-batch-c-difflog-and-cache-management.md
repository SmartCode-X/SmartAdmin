...
- 状态:已采纳(2026-07-18)
- 相关:`skills/replace-service`(消费者替换成法)

## 背景

本批次原设想两件:C1 实体变更差异日志(SqlSugar DiffLog),C2 缓存管理页(键浏览 + 清除)。开工前用 `/grill-with-docs` 做了一轮设计审问 + 代码探查 + SqlSugar 官方文档核对,结论与原设想差距很大,记两条决策防反复。
