...
//    打开即执行 → 偷 localStorage 里的 JWT 达成账号接管。**「存纯 Markdown」不等于无 XSS 面**
//    (Markdown 天然放行 HTML),所以挂库自带 `XSSPlugin`(基于已随库打进来的 js-xss,零新依赖)。
...
let done = false
/**
 * 幂等:全局登记 XSS 过滤 + echarts 空 instance(断 unpkg)。
 * 由 MarkdownEditor / MarkdownView 在 setup 首行调用 —— 放启动入口(createSmartAdmin)会把 md-editor-v3 拖进首屏 chunk。
 */
