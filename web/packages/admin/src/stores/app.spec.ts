...
describe('useAppStore.resetSettings', () => {
  it('外观项回默认,但 locale / collapsed 保留', () => {
    ...
    expect(app.density).toBe('compact') // 恢复默认后，系统设置密度应回到紧凑档。
...
// 把 pinia 正经装一遍,复刻 createSmartAdmin 的真实接线顺序,afterHydrate 才会真的跑到。
...
describe('useAppStore persist.afterHydrate 校验持久化值', () => {
  it('layoutMode 不在合法集合内 → 回落 vertical;合法值不动', () => {
    vi.stubGlobal('localStorage', makeMemoryStorage())

    localStorage.setItem('app', JSON.stringify({ layoutMode: 'mixed-nav' })) // 不在 LAYOUT_MODES 里
    const app1 = mountAppStore(createPinia())
    expect(app1.layoutMode).toBe('vertical')

    localStorage.setItem('app', JSON.stringify({ layoutMode: 'horizontal' })) // 合法值
    const app2 = mountAppStore(createPinia())
    expect(app2.layoutMode).toBe('horizontal')
  })

  it('accent 是任一候选色都原样保留;不在 ACCENTS 里 → 回落默认', () => {
    vi.stubGlobal('localStorage', makeMemoryStorage())

    for (const accent of ACCENTS) {
      localStorage.setItem('app', JSON.stringify({ accent }))
      expect(mountAppStore(createPinia()).accent).toBe(accent)
    }

    localStorage.setItem('app', JSON.stringify({ accent: '#0082CE' })) // 不在 ACCENTS 里
    expect(mountAppStore(createPinia()).accent).toBe(ACCENTS[0])
  })
})
