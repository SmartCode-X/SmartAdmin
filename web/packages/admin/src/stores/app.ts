...
/** 布局模式全集(单一来源:抽屉卡片、布局 shell、持久化值校验共用)。 */
...
const DEFAULTS = {
  ...
  density: 'compact' as Density, // 系统设置密度默认使用紧凑档，提升后台列表与表单的信息密度。
...
export const useAppStore = defineStore('app', {
  ...
  actions: {
    ...
    exportSettings(): Record<keyof typeof DEFAULTS, unknown> {
      ...
      return Object.fromEntries(keys.map(k => [k, this[k]])) as Record<
  ...
  persist: {
    // localStorage 里的值不受代码约束:布局模式不在 LAYOUT_MODES、主色不在 ACCENTS 的,回落默认。
    afterHydrate: ({ store }) => {
