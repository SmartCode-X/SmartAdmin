...
<template>
  <div class="aurora" :class="{ light: !app.isDark }" :style="vars">
    ...
    <div class="aurora-content">
      ...
      <n-config-provider
        :theme="naiveTheme"
        :theme-overrides="glassTheme"
        :inherit-theme-overrides="false"
      >
...
.aurora {
  ...
  background: radial-gradient(140% 120% at 50% 0%, #12162b 0%, #0a0b12 55%, #06060c 100%);
}
/* 细网格,中心遮罩淡出 */
.aurora::before {
  content: '';
...
/* ── 浅色变体(应用浅色主题时)──────────────────────── */
.aurora.light {
  background: radial-gradient(140% 120% at 50% 0%, #eef1fb 0%, #f6f7fc 55%, #ffffff 100%);
}
/* 网格线改深(浅底上可见) */
.aurora.light::before {
  background-image:
