...
// 校验落在表单项上(全站一致),不用 toast —— toast 飘在角落、不指向出错的那一栏,且会被后续消息挤掉。
// ponytail: 密码复杂度不进 rules。PasswordStrength 已在输入时逐条实时显示策略清单,后端也强制(弱口令返 passwordTooWeak),
// 再抄一份到 rules 就是第三处真源,策略一改三处都要跟。
const rules: FormRules = {
  oldPassword: {
    required: true,
    message: () => t('changePassword.oldRequired'),
    trigger: ['input', 'blur'],
  },
  newPassword: {
    required: true,
    message: () => t('changePassword.newRequired'),
    trigger: ['input', 'blur'],
  },
  confirmPassword: {
    required: true,
    ...
    validator: (_rule, value: string) =>
      !value
        ? new Error(t('changePassword.confirmRequired'))
...
async function submit() {
  ...
  saving.value = true
  try {
    await personalApi.updatePassword({
...
<template>
  <n-card :bordered="true" :title="t('changePassword.title')" style="max-width: 460px">
    ...
    <n-form ref="formRef" :model="model" :rules="rules" label-placement="top">
      ...
      <!-- 不用 :disabled 做第二重必填:必填已经是 rules 的事,禁用按钮反而让人不知道缺什么 -->
