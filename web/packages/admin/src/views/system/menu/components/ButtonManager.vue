...
// 某个菜单/目录下「权限按钮」的专属管理弹窗 —— 按钮不进主菜单树、在这里单独管理,
// 树里只放目录/菜单,不被几十个按钮撑长。按钮就是 type=Button 的 SysMenu 行,
// 增删改复用 menuApi.add/update/remove(parentId=当前菜单),没有专用端点。
...
/** 已占用的权限码(多码按钮展开成单条),用于批量添加去重。 */
const usedCodes = computed(() => new Set(buttons.value.flatMap(b => splitPermission(b.permission))))
...
const hasOtherRoutes = computed(
  () => routeSeg.value !== '' && props.routes.some(r => !belongsToApp(r.path)),
)
const showAllRoutes = ref(false)

// 权限码下拉(多选):label 展示「METHOD /路径」便于辨认,value 就是要写进 permission 的码本身。
...
// 权限码在表单里按数组编辑(一颗按钮可挂多条路由),保存时拼回 form.permission 的 ; 串。
...
function openEdit(r: MenuTreeNode) {
  ...
  permissionCodes.value = splitPermission(r.permission)
  editShow.value = true
}
async function save() {
  ...
  try {
    ...
    const payload: MenuInput = {
      ...form,
      permission: joinPermission(permissionCodes.value),
      type: MenuType.Button,
      moduleId: null,
    }
  ...
  { title: () => t('menu.title'), key: 'title', align: 'left' },
  {
    ...
    render: r => {
      ...
      return h(NSpace, { size: 4, wrapItem: false }, () =>
        codes.map(c => h(NTag, { size: 'small', bordered: false, key: c }, () => c)),
      )
    },
  },
...
<template>
  ...
  >
    <n-space vertical :size="12">
      <n-space>
        <n-button v-if="canAdd" type="primary" size="small" @click="openAdd">
          {{ t('menu.addButton') }}
        </n-button>
        <n-button v-if="canAdd" size="small" @click="openBatch">
          {{ t('menu.batchFromRoutes') }}
        </n-button>
      </n-space>
      <SmartTable
      ...
      <n-grid :cols="1" :y-gap="4">
        ...
        <n-form-item-gi :label="t('menu.permission')">
          <n-space vertical :size="4" style="width: 100%">
            ...
            />
            <span class="perm-hint">{{ t('menu.permissionHint') }}</span>
  ...
  >
    <n-empty
      ...
      :description="t('menu.noRoutesLeft')"
    ...
    <n-space v-else vertical :size="10">
      <n-space align="center" :size="12">
        ...
        <n-checkbox v-if="hasOtherRoutes" v-model:checked="showAllRoutes">
          {{ t('menu.showAllRoutes') }}
        </n-checkbox>
      </n-space>
      <n-empty
        v-if="!filteredBatchRows.length"
        :description="t('menu.noRoutesLeft')"
        style="padding: 16px 0"
      />
      <n-space v-else vertical :size="6" style="max-height: 55vh; overflow: auto">
        <div
          v-for="r in filteredBatchRows"
          :key="r.code"
          style="display: flex; align-items: center; gap: 10px"
        >
          <n-checkbox v-model:checked="r.checked" />
          <span style="width: 210px; color: var(--color-text-secondary, #888); font-size: 12px">
            {{ r.method }} {{ r.path }}
          </span>
          <n-input
