...
// 菜单「新增/编辑」弹窗。父页(menu/index.vue)拥有 tree/modules 数据并保持响应,
...
// 类型只留目录/页面:按钮统一走「权限按钮」弹窗建,主表单不提供按钮选项。
...
// 组件路径下拉:取自已注册的页面表(内核内置页 + 消费方注册的页面),选得到的一定存在;手填错误则由 MissingRoute 诊断。
...
const parentOptions = computed(() => {
  const exclude =
    editingId.value === null ? new Set<number>() : subtreeIds(props.tree, editingId.value)
...
function openAdd(parentId = 0) {
  ...
  const moduleId =
    parentId === 0 && props.moduleFilter !== UNASSIGNED && props.moduleFilter !== ALL_MODULES
      ? props.moduleFilter
      : null
  ...
  >
    ...
    <n-form ref="formRef" :model="form" :rules="rules" label-placement="left" :label-width="90">
      <n-grid :cols="2" :x-gap="18">
        ...
        <!-- 无权限码字段:目录/页面结构上永远无权限码(授权靠角色拥有菜单,页面是前端路由无后端端点),
        ...
        <!-- 组件路径:选项来自已注册的页面表(内核内置页 + createSmartAdmin 传入的页面),选得到的一定能加载;
