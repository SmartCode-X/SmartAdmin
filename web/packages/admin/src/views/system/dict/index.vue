...
// 字典管理 = 主从:左=类型 SmartTable(CRUD),点击行选中 → 右=该类型的字典项(裸 n-data-table + CRUD)。
...
const {
  checkedKeys: typeCheckedKeys,
  hasSelection: typeHasSelection,
  run: typeBatchDelete,
} = useBatchDelete({
    ...
    render: r =>
      h(NSpace, { size: 4, wrapItem: false, onClick: (e: Event) => e.stopPropagation() }, () => [
        authStore.hasPerm('PUT:/api/v1/sys/dict/type/{id}')
          ? h(
              NButton,
              { size: 'small', quaternary: true, type: 'primary', onClick: () => openTypeEdit(r) },
              () => t('common.edit'),
            )
          : null,
...
<template>
  <!-- 两栏并排 = styles/layout.css 的形状 3(.side-page):display/拉伸/整屏高度都在那儿。
       两个面板外面各套了一层自己的卡片,故卡片挂 .fill-card 接高度链;
       左栏里还嵌着 SmartTable(卡中卡),「包住主表的那一层」就是卡片内容区,故 content-class 标 .fill-main
       —— 契约里写明了别把它写在 <SmartTable> 上(inheritAttrs:false,class 到不了 .smart-table 根节点)。 -->
  <div class="dict-layout side-page">
    <n-card class="dict-pane fill-card" content-class="fill-main" :bordered="true">
      <SmartTable
    ...
    >
      <template #header-extra>
        <n-space :size="8">
          ...
          >
            <template #icon><AppIcon icon="ph:trash" :size="15" /></template>
            {{ t('common.batchDelete') }}
          </n-button>
          <n-button
            ...
            @click="openItemAdd"
          >
            <template #icon><AppIcon icon="ph:plus" :size="15" /></template>
            {{ t('dict.addItem') }}
          </n-button>
        </n-space>
      </template>
