...
// 用户「新增/编辑」弹窗。职位/角色/主管下拉由父页(user/index.vue)拉好经 props 传入(避免重复请求);
...
const rules: FormRules = {
  account: {
    required: true,
    whitespace: true,
    message: () => t('user.accountRequired'),
    trigger: ['input', 'blur'],
  },
  name: {
    required: true,
  ...
  phone: {
    validator: (_r, v: string) => !v || /^1[3-9]\d{9}$/.test(v),
    message: () => t('user.phoneInvalid'),
    trigger: ['input', 'blur'],
  },
  email: {
    validator: (_r, v: string) => !v || /^[^@\s]+@[^@\s]+\.[^@\s]+$/.test(v),
    ...
    <n-form ref="formRef" :model="form" :rules="rules" label-placement="left" :label-width="96">
      <n-grid :cols="2" :x-gap="16">
        ...
        <n-form-item-gi :label="t('user.director')">
          <n-select
            v-model:value="form.directorId"
            :options="props.directorOptions"
            clearable
            filterable
            :placeholder="t('user.directorPlaceholder')"
          />
        </n-form-item-gi>
