...
// 机构管理 = 树表:SmartTable 静态数据 + children 树模式(无分页)。
...
const filteredTree = computed(() => {
  ...
  return filterTree(
    tree.value,
    n => n.name.toLowerCase().includes(kw) || n.code.toLowerCase().includes(kw),
  )
})
...
watch(filteredTree, next => (expandedKeys.value = expandableIds(next)), { immediate: true })

const allExpanded = computed(() => expandedKeys.value.length > 0)
const toggleExpandAll = () =>
  (expandedKeys.value = allExpanded.value ? [] : expandableIds(filteredTree.value))
  ...
  // 操作收敛成「编辑 + 更多▾」:4 个操作塞 260px 会换行、行高参差,且没 fixed 时横向滚动够不着。
  {
    ...
    render: r => {
      ...
      const dropdownOptions = [
        authStore.hasPerm('POST:/api/v1/sys/org/add')
          ? { key: 'addChild', label: t('org.addChild') }
          : null,
        authStore.hasPerm('POST:/api/v1/sys/org/{id}/copy')
          ? { key: 'copy', label: t('org.copy') }
          : null,
        authStore.hasPerm('DELETE:/api/v1/sys/org/{id}')
