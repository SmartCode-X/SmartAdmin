...
async function add() {
  ...
  saving.value = true
  try {
    await highSensApi.add({
      permissionCode: newCode.value.trim(),
...
const customCols: DataTableColumns<(typeof customs.value)[0]> = [
  {
    title: () => t('config.security.highSens.code'),
    key: 'permissionCode',
...
<template>
  <div>
    ...
    <n-data-table :columns="customCols" :data="customs" :loading="loading" size="small" />
  </div>
</template>
