...
const CLAIMED_KEYS = FIELDS.map(f => f.key)
...
async function save() {
  ...
  try {
    ...
    // 即时生效:重取全站共用的站点信息,侧栏/顶栏/登录页品牌与浏览器标题(usePageTitle 跟着 site.title)随之更新。
    await loadSite(true)
  } catch (e) {
    message.error(translateError(e))
  } finally {
...
<template>
  ...
  <GroupExtraConfig group="sys" :claimed="CLAIMED_KEYS" />
</template>
