...
const CLAIMED_KEYS = [KEY_MAX_SIZE, KEY_ALLOWED]
...
onMounted(async () => {
  try {
    ...
    exts.value = (map.get(KEY_ALLOWED) ?? '')
      .split(',')
      .map(e => e.trim())
      .filter(Boolean)
  } catch (e) {
...
<template>
  <n-spin :show="loading">
    <n-form label-placement="left" :label-width="150" style="max-width: 560px">
      ...
      <n-form-item :label="t('config.upload.allowedExtensions')">
        <n-dynamic-tags
          v-model:value="exts"
          :input-props="{ placeholder: t('config.upload.extPlaceholder') }"
        />
      </n-form-item>
  ...
  <GroupExtraConfig group="upload" :claimed="CLAIMED_KEYS" />
</template>
