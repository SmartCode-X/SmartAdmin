...
// 第三方登录运营 Tab:预置品牌全展示;未部署/未配密钥的不可打开卡片上的开关。
// 每张卡片有「登录页显示」;企业微信卡片另有「按账号自动关联」(见 LINK_BY_ACCOUNT_CODES)。
...
import { configApi, externalAuthApi, type ExternalProviderAdmin } from '#/api'
import {
  buildConfigProviderRows,
  LINK_BY_ACCOUNT_CODES,
  type ConfigProviderRow,
} from '#/utils/oauthBrand'
import { translateError } from '#/utils/error'
import GroupExtraConfig from './GroupExtraConfig.vue'

const { t } = useI18n()
const message = useMessage()
const { ask } = useConfirm()
const loading = ref(true)
// 首次载入完成前行表只有预置品牌,已注册的其它 provider 的开关键会被当成「本组其它配置」闪一下
const loaded = ref(false)
const saving = ref(false)
// 原始 providers 存 ref、行表用 computed:内置品牌名走 i18n,切语言要跟着重算。
const providers = ref<ExternalProviderAdmin[]>([])
const rows = computed<ConfigProviderRow[]>(() => buildConfigProviderRows(providers.value))

const enabledKey = (code: string) => `sys.externalauth.${code}.enabled`
const linkKey = (code: string) => `sys.externalauth.${code}.linkByAccount`
const hasLinkSwitch = (code: string) => LINK_BY_ACCOUNT_CODES.includes(code)
// 卡片上开关对应的键由卡片认领,不出现在「本组其它配置」里
const claimedKeys = computed(() =>
  rows.value.flatMap(p =>
...
const linkMap = ref<Record<string, boolean>>({})
const linkBaseline = ref<Record<string, boolean>>({})
...
const dirty = computed(() =>
  rows.value
    ...
    .some(
      p =>
        !!enabledMap.value[p.code] !== !!baseline.value[p.code] ||
        (hasLinkSwitch(p.code) && !!linkMap.value[p.code] !== !!linkBaseline.value[p.code]),
    ),
)
...
async function load() {
  loading.value = true
  try {
    providers.value = await externalAuthApi.providersAll()
    const m: Record<string, boolean> = {}
    const l: Record<string, boolean> = {}
    for (const p of rows.value) {
      m[p.code] = p.registered ? p.enabled : false
      l[p.code] = p.registered ? p.linkByAccount : false
    }
    enabledMap.value = { ...m }
    baseline.value = { ...m }
    linkMap.value = { ...l }
    linkBaseline.value = { ...l }
  } catch (e) {
...
async function save() {
  saving.value = true
  try {
    // 只写已注册项;未配置的永不写成 true
    const payload = rows.value
      .filter(p => p.registered)
      .flatMap(p => [
        { configKey: enabledKey(p.code), configValue: String(!!enabledMap.value[p.code]) },
        ...(hasLinkSwitch(p.code)
          ? [{ configKey: linkKey(p.code), configValue: String(!!linkMap.value[p.code]) }]
          : []),
      ])
...
function reset() {
  ...
  linkMap.value = { ...linkBaseline.value }
}
...
// 打开前先确认前提:关联上就是按账号名把人认下来,两边账号不同源会张冠李戴
async function onToggleLink(p: ConfigProviderRow, v: boolean) {
  ...
  if (
    ...
    return
  linkMap.value = { ...linkMap.value, [p.code]: v }
}
</script>
...
<template>
  <n-spin :show="loading">
    <div class="ea">
      ...
      <n-empty
        ...
        :description="t('config.externalAuth.empty')"
        class="ea-empty"
      />
      ...
      <template v-else-if="rows.length">
        ...
        <div class="ea-grid">
          ...
          >
            <div class="ea-card-main">
              ...
              <div class="ea-meta">
                <div class="ea-title-row">
                  ...
                  <n-tag v-if="!p.registered" size="small" type="warning" :bordered="false" round>
                ...
                <p v-if="!p.registered" class="ea-na-tip">
                  {{ t('config.externalAuth.notConfiguredTip') }}
                </p>
              </div>
            </div>
            <div class="ea-switch">
            ...
            <div v-if="hasLinkSwitch(p.code)" class="ea-switch">
              <span class="ea-switch-label">{{ t('config.externalAuth.linkByAccount') }}</span>
              <n-switch
                :value="!!linkMap[p.code]"
                :disabled="!p.registered"
                size="medium"
                @update:value="v => onToggleLink(p, v)"
              />
            </div>
          </div>
        </div>
  ...
  <GroupExtraConfig v-if="loaded" group="externalauth" :claimed="claimedKeys" />
</template>
...
/* 同一张卡片的第二个开关与「登录页显示」共用底栏,不另画分隔线 */
.ea-switch + .ea-switch {
  margin-top: -4px;
  padding-top: 0;
  border-top: 0;
