...
const NUM_FIELDS = [
  ...
  { key: 'sys.security.password.historyCount', min: 0 }, // 0 = 关闭防重用
...
// 短信验证两开关(登录加固):二次验证(密码后再验短信码)/ 免密登录(手机号+码)。
...
// 分节单独渲染的开关,载入/保存与 BOOL_FIELDS 同一套布尔处理
...
const SWITCH_KEYS = [...BOOL_FIELDS, ...SECTION_SWITCHES]
// 本 Tab 绑定的全部键:保存按它们回写,「本组其它配置」按它们排除
const CLAIMED_KEYS = [...NUM_FIELDS.map(f => f.key), ...SWITCH_KEYS, CAPTCHA_TYPE_KEY]
...
onMounted(async () => {
  try {
    ...
    for (const k of SWITCH_KEYS) bools[k] = map.get(k) === 'true'
...
async function save() {
  ...
  try {
    ...
    await configApi.saveBatch([
      ...
      ...SWITCH_KEYS.map(k => ({ configKey: k, configValue: String(bools[k]) })),
...
<template>
    ...
    <n-form label-placement="top" style="max-width: 960px">
      ...
      <n-grid :cols="'1 s:2'" responsive="screen" :x-gap="32" :y-gap="0">
        <n-form-item-gi :label="label('sys.security.loginLock.maxFailCount')">
          <n-input-number
            v-model:value="nums['sys.security.loginLock.maxFailCount']"
            :min="0"
            style="width: 160px"
          />
        </n-form-item-gi>
        <n-form-item-gi :label="label('sys.security.loginLock.lockMinutes')">
          <n-input-number
            v-model:value="nums['sys.security.loginLock.lockMinutes']"
            :min="1"
            style="width: 160px"
          />
        </n-form-item-gi>
      ...
      <n-grid :cols="'1 s:2'" responsive="screen" :x-gap="32" :y-gap="0">
        <n-form-item-gi :label="label('sys.security.password.minLength')">
          <n-input-number
            v-model:value="nums['sys.security.password.minLength']"
            :min="1"
            style="width: 160px"
          />
        </n-form-item-gi>
        <n-form-item-gi :label="label('sys.security.password.expireDays')">
          <n-input-number
            v-model:value="nums['sys.security.password.expireDays']"
            :min="0"
            style="width: 160px"
          />
        </n-form-item-gi>
        <n-form-item-gi :label="label('sys.security.password.historyCount')">
          <n-input-number
            v-model:value="nums['sys.security.password.historyCount']"
            :min="0"
            style="width: 160px"
          />
        </n-form-item-gi>
      ...
      <n-grid :cols="'1 s:2'" responsive="screen" :x-gap="32" :y-gap="0">
        <n-form-item-gi :label="label('sys.security.session.accessMinutes')">
          <n-input-number
            v-model:value="nums['sys.security.session.accessMinutes']"
            :min="1"
            style="width: 160px"
          />
        </n-form-item-gi>
        <n-form-item-gi :label="label('sys.security.session.refreshMinutes')">
          <n-input-number
            v-model:value="nums['sys.security.session.refreshMinutes']"
            :min="1"
            style="width: 160px"
          />
        </n-form-item-gi>
      ...
      <n-grid :cols="'1 s:2'" responsive="screen" :x-gap="32" :y-gap="0">
        ...
        <n-form-item-gi :label="label('sys.security.rateLimit.windowSeconds')">
          <n-input-number
            v-model:value="nums['sys.security.rateLimit.windowSeconds']"
            :min="1"
            style="width: 160px"
          />
        </n-form-item-gi>
        <n-form-item-gi :label="label('sys.security.rateLimit.permitPerWindow')">
          <n-input-number
            v-model:value="nums['sys.security.rateLimit.permitPerWindow']"
            :min="0"
            style="width: 160px"
          />
        </n-form-item-gi>
        <n-form-item-gi :label="label('sys.security.rateLimit.authPermitPerWindow')">
          <n-input-number
            v-model:value="nums['sys.security.rateLimit.authPermitPerWindow']"
            :min="0"
            style="width: 160px"
          />
        </n-form-item-gi>
  ...
  <GroupExtraConfig group="security" :claimed="CLAIMED_KEYS" />
</template>
