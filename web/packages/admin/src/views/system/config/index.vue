...
// 分类配置中心:按分类 Tab 组织,一个结构化 Tab 管一个分组(见 groups.ts)。系统基础=结构化表单(字段级 UI);
// 安全策略=锁定/密码/会话结构化表单(后端 ISecurityPolicyProvider 强制执行,改值即时生效);
// 结构化 Tab 下方的「本组其它配置」列同组里没被字段认领的行;其他配置=不归任何结构化 Tab 的分组的扁平 CRUD 兜底。
