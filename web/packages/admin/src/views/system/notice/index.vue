...
import {
  ...
  ReceiverType,
  type NoticeAction,
  type NoticePublishInput,
  type SysNotice,
} from '#/types/api'
import { isSiteRelativeUrl } from '#/lib/noticeActions'
...
const columns: SmartTableColumn<SysNotice>[] = [
  ...
  { key: 'title', title: () => t('notice.noticeTitle'), search: true },
  {
    ...
    width: 90,
    render: r =>
      h(NTag, { size: 'small', type: tagType(r.type), bordered: false }, () => typeLabel(r.type)),
  },
  { key: 'content', title: () => t('notice.content'), ellipsis: { tooltip: true } },
  {
    key: 'actionsJson',
    title: () => t('notice.actions'),
    width: 100,
    render: r => {
      const n = actionCount(r.actionsJson)
      return n ? t('notice.actionCount', { count: n }) : '—'
    },
  },
  {
    key: 'createTime',
...
const rules: FormRules = {
  ...
  actions: {
    // 与后端同一条规则:文案非空、站内路径单个 / 开头;服务端仍会再校验一遍(45004)
    validator: () => form.actions.every(a => a.label.trim() && isSiteRelativeUrl(a.url.trim())),
    message: () => t('error.notice.actionInvalid'),
    trigger: ['input', 'blur'],
  },
}
// 表单里 actions 恒为数组(n-dynamic-input 不吃 null);提交时空数组不传
type PublishForm = Omit<NoticePublishInput, 'actions'> & { actions: NoticeAction[] }
const blank = (): PublishForm => ({
  title: '',
  content: '',
...
const form = reactive<PublishForm>(blank())

// ── 动作按钮编辑(最多 5 条) ──
const MAX_ACTIONS = 5
const newAction = (): NoticeAction => ({
  label: '',
  method: 'GET',
  url: '',
  style: 'default',
...
const styleOptions = [
  { label: () => t('notice.actionStyleDefault'), value: 'default' },
  { label: () => t('notice.actionStylePrimary'), value: 'primary' },
  { label: () => t('notice.actionStyleError'), value: 'error' },
]
/** 管理端列表只展示条数;JSON 坏了按 0 算,不让一行脏数据拖垮整页。 */
function actionCount(json?: string | null): number {
  if (!json) return 0
  try {
...
const receiverOptions = computed(() =>
  form.receiverType === ReceiverType.Role
    ? roleOptions.value
    : form.receiverType === ReceiverType.User
      ? userOptions.value
      : [],
)
...
async function savePublish() {
  ...
  try {
    // 空列表不传:后端 null 与 [] 同义,但 schema 契约里它是可选字段,别把 [] 当成"有动作"
    const actions = form.actions.length ? form.actions : undefined
    await noticeApi.publish({ ...form, actions })
    ...
    >
      <template #toolbar>
        <n-button v-auth="'POST:/api/v1/sys/notice'" type="primary" @click="openPublish">
          <template #icon><AppIcon icon="ph:megaphone" :size="16" /></template>
          {{ t('notice.publish') }}
        </n-button>
      </template>
    </SmartTable>
    ...
    >
      <n-form ref="formRef" :model="form" :rules="rules" label-placement="left" :label-width="80">
        ...
        <n-form-item
          ...
          :label="
            form.receiverType === ReceiverType.Role
              ? t('notice.receiverRole')
              : t('notice.receiverUser')
          "
        >
          <n-select
        ...
        <n-form-item :label="t('notice.actions')" path="actions">
          <div class="notice-actions-editor">
            <n-dynamic-input
              ...
              <template #default="{ value }">
                <div class="notice-action-row">
                  <n-input
                    ...
                    :placeholder="t('notice.actionLabel')"
                    style="width: 150px"
                  />
                  <n-select
                  ...
                  <n-input
                    ...
                    :placeholder="t('notice.actionUrl')"
                  ...
                  <n-select
                    ...
                    style="width: 96px"
                  />
                  <n-checkbox v-model:checked="value.confirm">
                    {{ t('notice.actionNeedConfirm') }}
                  </n-checkbox>
                  <n-checkbox v-model:checked="value.comment">
                    {{ t('notice.actionNeedComment') }}
                  </n-checkbox>
                </div>
              </template>
              <template #create-button-default>{{ t('notice.actionAdd') }}</template>
            </n-dynamic-input>
            <n-text depth="3" class="notice-actions-hint">{{ t('notice.actionsHint') }}</n-text>
          </div>
        </n-form-item>
      </n-form>
    </FormContainer>
