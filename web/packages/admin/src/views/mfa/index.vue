    ...
    <n-card class="mfa-card" :bordered="true">
      ...
      <template v-if="mode === 'bind'">
        ...
        <n-form v-if="step === 1" label-placement="top" @keyup.enter="startBind">
          ...
          <n-button type="primary" block :loading="loading" @click="startBind">
            {{ t('mfa.startBind') }}
          </n-button>
        </n-form>
        ...
        <n-form v-else-if="step === 2" label-placement="top" @keyup.enter="completeBind">
          ...
          <n-button type="primary" block :loading="loading" @click="completeBind">
            {{ t('mfa.completeBind') }}
          </n-button>
        </n-form>
        ...
        <div v-else class="recovery-codes">
          ...
          <n-button type="primary" block style="margin-top: 16px" @click="downloadRecoveryCodes">
            {{ t('mfa.downloadCodes') }}
          </n-button>
          <n-button block style="margin-top: 8px" @click="copy(bind.recoveryCodes.join('\n'))">
            {{ t('mfa.copyAll') }}
          </n-button>
          <n-button block style="margin-top: 8px" @click="backToLogin">
            {{ t('mfa.backToLogin') }}
          </n-button>
        </div>
      </template>
      ...
      <n-form v-else-if="!completed" label-placement="top" @keyup.enter="submitRecovery">
        ...
        <n-button type="primary" block :loading="loading" @click="submitRecovery">
          {{ t('mfa.submitRecovery') }}
        </n-button>
      </n-form>
      <template v-else>
        <n-alert type="success" :bordered="false">{{ t('mfa.recoveryDone') }}</n-alert>
        <n-button type="primary" block style="margin-top: 16px" @click="backToLogin">
          {{ t('mfa.backToLogin') }}
        </n-button>
      </template>
    </n-card>
...
.mfa-page {
  ...
  place-items: center;
  padding: 24px;
  background: var(--color-body, #f5f7fa);
}
.mfa-card {
  width: min(100%, 520px);
}
...
.recovery-codes :deep(.n-alert),
.recovery-codes :deep(.n-button) {
  grid-column: 1 / -1;
}
.recovery-code {
  padding: 8px 10px;
