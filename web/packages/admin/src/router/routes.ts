...
export const staticRoutes: RouteRecordRaw[] = [
  ...
  // 外部登录 / SSO 回调结果页:IdP → 后端回调 → 302 到此,凭票据换令牌或提示错误。
  ...
  // 门户重建失败(网络/5xx/限流)的落点。必须在布局壳之外:到这里时动态路由恰恰没建起来。
  {
    path: '/boot-error',
    name: 'boot-error',
    component: () => import('#/views/error/BootError.vue'),
  },
  {
    path: '/module',
    name: 'module',
  ...
  // 站内整屏页(/fullscreen/*):脱离布局壳占满窗口,给挂墙大屏 / 看板用。消费方的整屏路由经
  // createSmartAdmin({ routes }) 注册为顶级路由(与这里同级);菜单节点 path 填该路径、component 留空 →
  // 点击走 window.open 新标签(见 utils/url.isFullscreenPath)。非 public:仍走登录守卫。
  {
    path: '/',
    name: 'layout',
    component: () => import('#/layouts/default.vue'),
    // 不设 redirect:它在 resolve 阶段求值,早于全局守卫——那时动态路由还没重建、menuTree 还空,
    // 算出的首页必然是错的。'/' 的落点由守卫在路由就绪后决定(见 router/index.ts)。
    children: [
      ...
      // 个人中心:二级壳(左导航)+ 子页;URL 仍为 /personal/*。通知铃铛入口独立,不进中心导航。
      {
        ...
        children: [
          ...
          },
          {
            ...
            component: namedPage(
              'personal-password',
              () => import('#/views/personal/password.vue'),
            ),
          ...
          },
          {
            ...
            component: namedPage(
              'personal-security',
              () => import('#/views/personal/security.vue'),
            ),
          ...
          },
          {
            ...
            component: namedPage(
              'personal-sessions',
              () => import('#/views/personal/sessions.vue'),
            ),
          ...
          },
          {
            ...
            component: namedPage(
              'personal-bindings',
              () => import('#/views/personal/bindings.vue'),
            ),
