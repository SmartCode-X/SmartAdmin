// 颜色派生规则(DESIGN.md 权威实现)。纯函数,无框架依赖。
...
/**
 * accent → 主色四态(DESIGN.md)。暗色先把 accent 提亮一档,再派生 hover/pressed/light。
 *
 * 派生只在这一处:`naive-theme.ts`(取前三态,喂 Naive overrides)与 `useTheme.ts` 的 `applyPrimaryVars`
 * (取四态,写 CSS 变量)都调它。两边各算一份的话,改一处忘另一处 = 裸 CSS 与 Naive 组件主色不同步,
 * **而没有任何东西会报错**。
 *
