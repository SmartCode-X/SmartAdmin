...
// 四步导入向导:①上传 ②列映射 ③预览改错(裸 n-data-table)④结果。
...
const props = defineProps<{
  api: ImportWizardApi
  /**
   * 弹窗标题。缺省是 `import.wizardTitle`(「导入用户」)——那是内核用户导入的文案,
   * 本组件被别的档案复用时必须传自己的标题,否则会顶着「导入用户」导别的东西。
   */
  title?: string
  /** 模板下载默认文件名 */
  ...
  /**
   * 可选的重复策略,按给定顺序渲染;缺省三种都给。
   * 用于档案本身不支持某种策略的场景 —— 例如条码储位只允许「跳过已存在」,
   * 覆盖会把条码挪到别的库位,不是导入该干的事。
   */
  strategies?: DupStrategyNum[]
}>()
...
const columns = shallowRef<ImportColumn[]>([])
// rows 刻意保持深响应:setCell 是就地改 row.cells / row.errors(可编辑预览表),
// 换成 shallowRef 后单元格改完不会重渲,红底也不会消 —— 要改得先把编辑改成不可变替换。
...
/** 策略标签用函数形式,切换语言即时生效(与 SmartTable 列 title 同理)。 */
const strategyLabels: Record<DupStrategyNum, () => string> = {
  [DuplicateStrategy.Skip]: () => t('import.strategySkip'),
  [DuplicateStrategy.Overwrite]: () => t('import.strategyOverwrite'),
  [DuplicateStrategy.Error]: () => t('import.strategyError'),
}
/** 未指定 strategies 时三种都给。 */
...
function reset() {
  ...
  // 默认选 Skip;调用方限制了可选策略且不含 Skip 时退到第一个可选项,避免选中一个没渲染的值。
  strategy.value = strategyOptions.value.some(o => o.value === DuplicateStrategy.Skip)
    ? DuplicateStrategy.Skip
    : (strategyOptions.value[0]?.value ?? DuplicateStrategy.Skip)
...
async function goMapping() {
  ...
  try {
    ...
    for (const header of p.headers) {
      if (!(header in mapping.value)) mapping.value[header] = ''
    }
    step.value = 2
  } catch (e) {
    message.error(translateError(e))
      ...
      render: row => {
        ...
        return h(
          ...
          { trigger: 'hover' },
          {
            trigger: () => cell,
            default: () =>
              soft ? t(`import.dupHint.${strategy.value}`) : translateError(err.code),
          },
        )
      },
    })
  ...
  cols.push({
    ...
    render: r => {
      if (r.errors.length === 0)
        return h(NTag, { size: 'small', type: 'success', bordered: false }, () => t('import.ok'))
...
<template>
  <n-modal
    v-model:show="show"
    preset="card"
    :title="props.title ?? t('import.wizardTitle')"
  ...
  >
    ...
    <div v-show="step === 2" class="import-pane">
      <n-alert v-if="columnErrors.length" type="warning" style="margin-bottom: 12px">
        {{ columnErrors.map(e => `${e.columnKey}: ${translateError(e.code)}`).join('; ') }}
      </n-alert>
    ...
    <!-- ③ 预览改错(裸 n-data-table,不用 SmartTable) -->
    <div v-show="step === 3" class="import-pane">
      ...
      <div class="import-strategy">
        ...
        <n-radio-group v-model:value="strategy" name="dup-strategy">
          <n-space>
            <n-radio v-for="option in strategyOptions" :key="option.value" :value="option.value">
              {{ option.label() }}
            </n-radio>
          </n-space>
        </n-radio-group>
    ...
    <template #footer>
      <n-space justify="end">
        ...
        <n-button v-if="step === 3" secondary :disabled="loading" @click="downloadErrorReport">
          {{ t('import.downloadErrorReport') }}
        </n-button>
        <n-button v-if="step > 1 && step < 4" :disabled="loading" @click="step = step - 1">
          {{ t('import.prev') }}
        </n-button>
        <n-button
          v-if="step === 1"
        ...
        <n-button v-if="step === 2" type="primary" :loading="loading" @click="goPreview">
          {{ t('import.next') }}
        </n-button>
        <n-button v-if="step === 3" type="primary" :loading="loading" @click="doCommit">
          {{ t('import.commit') }}
        </n-button>
      </n-space>
    </template>
