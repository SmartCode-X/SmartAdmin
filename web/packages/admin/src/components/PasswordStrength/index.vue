...
// i18n 用 changePassword.* 键:它们只是键路径,为组件改名是纯 churn。
  ...
  const rows = [
    {
      key: 'minLength',
      ok: c.minLength,
      text: t('changePassword.rules.minLength', { n: pol.minLength }),
    },
  ]
  if (pol.requireUpper)
    rows.push({ key: 'upper', ok: c.upper, text: t('changePassword.rules.upper') })
  if (pol.requireLower)
    rows.push({ key: 'lower', ok: c.lower, text: t('changePassword.rules.lower') })
  if (pol.requireDigit)
    rows.push({ key: 'digit', ok: c.digit, text: t('changePassword.rules.digit') })
  rows.push({
    key: 'special',
    ok: c.special,
    text: pol.requireSpecial
      ? t('changePassword.rules.special')
      : t('changePassword.rules.specialOptional'),
  })
  return rows
})
</script>
