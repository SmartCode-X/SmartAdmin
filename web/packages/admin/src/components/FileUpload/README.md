...
| `upload`  | `(file: File) => Promise<FileUploadOutput>` | —       | 覆盖默认上传实现:传了就不再看 `chunked`,直接调这个函数上传。给需要走自定义接口的场景用。                                                                                                                                                    |
...
| `loadingChange` | `[loading: boolean]` | 单个文件上传的起止各触发一次(`try/finally` 兜底,失败也会收到 `false`)。给触发器加"上传中"态用。 |
