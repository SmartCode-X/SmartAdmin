...
- **预设**(`LineChart` / `BarChart` / `PieChart`):常规折线 / 柱状 / 饼,传 `data`,少写模板。
...
内核要支持地图 / 关系图 / 3D 等,在 `echarts.ts` 加一行 `use([...])` 即可。

## 用法

```vue
<script setup lang="ts">
import { Chart, LineChart, BarChart, PieChart } from 'smart-admin-web'
</script>
...
<template>
  ...
  <!-- 柱状:categories/series 结构与 LineChart 相同,无 smooth -->
  <BarChart
    :categories="['一', '二', '三', '四', '五']"
    :series="[{ name: '登录', data: [120, 132, 101, 134, 90] }]"
  />
...
**BarChart**:`categories: string[]`、`series: { name; data: number[] }[]`、`title?`(与 LineChart 同构,柱状不需要 `smooth`)。
