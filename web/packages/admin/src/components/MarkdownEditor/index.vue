...
// **天然放行内联 HTML**,不等于无 XSS 面;渲染安全由本文件 setup 首行 setupMarkdown() 挂的 XSSPlugin 兜底
...
// 在这里(而不是启动入口 createSmartAdmin)挂全局配置:放入口会把 md-editor-v3 + js-xss 整份拖进首屏 chunk。
// setupMarkdown 幂等,编辑器/只读视图谁先挂载都行。
setupMarkdown()
