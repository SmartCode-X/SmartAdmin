...
// 标题翻译走单源(locales/menuTitle):侧栏 / 面包屑 / 多标签 / 浏览器标题必须同一条规则。
...
function go(item?: Hit) {
  if (item) {
    // 外链与站内整屏页(/fullscreen/*，如大屏看板)一律新标签打开，与侧栏点击行为一致
    if (isHttpUrl(item.path) || isFullscreenPath(item.path))
...
function highlight(text: string) {
  ...
  if (i === -1) return safe
  return (
    safe.slice(0, i) + '<mark>' + safe.slice(i, i + k.length) + '</mark>' + safe.slice(i + k.length)
  )
}
      ...
      <n-scrollbar style="max-height: 52vh">
        <div v-if="results.length" class="list">
          <div
            ...
            @mouseenter="cursor = i"
          >
            <!-- 菜单图标来自库里配的名字,可能不在离线子集内 → 走 AppIcon 的懒加载兜底 -->
            <AppIcon v-if="item.icon" :icon="item.icon" :size="18" class="iicon" />
