...
配套约定式详情路由(`views/**/detail.vue` → `/<路径>/:id/detail`,见 `src/router/detailRoutes.ts`)与 `useTabTitle()`(数据加载后把标签标题改成记录名)。做法与骨架见 `skills/create-page-variant/detail.md`(内核未内置示例页;应用在自己的 `src/views/<模块>/` 下放一个 `detail.vue`,随 `createSmartAdmin({ views })` 进页面表即可)。
