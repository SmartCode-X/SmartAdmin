...
// 内核自带的品牌圆标(assets/oauth/),库构建时内联进产物
const assetModules = import.meta.glob('../../assets/oauth/*.{svg,png,webp}', {
  eager: true,
  import: 'default',
  query: '?url',
...
<template>
  ...
  <span
    ...
    aria-hidden="true"
  >
    <SmartIcon v-if="fallback?.icon" :icon="fallback.icon" :size="size" fallback="ph:link-simple" />
    <SmartIcon
    ...
    />
    <span v-else class="oauth-brand-letter">
