...
// SmartAdmin 图标选择器:复用 npm 包的 SmartIconPicker,仅把 SmartAdmin 的 i18n 文案注入 labels(包本身不带 i18n 框架)。
// 图标集/本地 SVG 由 createSmartAdmin 里的 setupIcons() 全局注册;这里不传 collections,复用全局注册顺序(ph 为首/默认页)。
import { computed } from 'vue'
import { useI18n } from 'vue-i18n'
import { SmartIconPicker } from 'smart-naive-icon'
...
const labels = computed(() => ({
  ...
  loading: t('common.loading'), // 包用 labels.loading;SmartAdmin 复用 common.loading
...
<template>
  <!-- 选择器 chrome 用 ph(这两个图标在启动时同步注册的离线子集里);SmartAdmin 语言包驱动全部文案 -->
  <SmartIconPicker
    v-model="model"
    :labels="labels"
    :clearable="clearable"
