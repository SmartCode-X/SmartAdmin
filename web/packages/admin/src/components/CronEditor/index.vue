...
// 后端语法为准(SmartAdmin.Core CronExpression):日段 L/L-n/LW/nW、周段 nL/n#m、周 0=周日(7≡0);
...
type SegMode =
  ...
  | 'last'
  | 'lastWeekday'
  | 'nearestWeekday' // 日段专项:L/L-n · LW · nW
  | 'lastOfWeek'
  | 'nth' // 周段专项:nL · n#m
...
const segs: SegDef[] = [
  {
    ...
    min: 0,
    max: 59,
    tab: () => t('job.cron.tabSecond'),
    listOptions: () => numOptions(0, 59),
  },
  {
    key: 'minute',
    st: states.minute,
  ...
  },
  {
    ...
    min: 0,
    max: 23,
    tab: () => t('job.cron.tabHour'),
    listOptions: () => numOptions(0, 23),
  },
  {
    key: 'day',
    st: states.day,
    min: 1,
    max: 31,
    tab: () => t('job.cron.tabDay'),
    listOptions: () => numOptions(1, 31),
  },
  {
    key: 'month',
    st: states.month,
    min: 1,
    max: 12,
    tab: () => t('job.cron.tabMonth'),
    listOptions: () => numOptions(1, 12),
  },
  {
    key: 'week',
    st: states.week,
    min: 0,
    max: 6,
    tab: () => t('job.cron.tabWeek'),
    listOptions: () => dowOptions.value,
  },
]
...
function build(seg: SegDef): string {
  ...
  switch (st.mode) {
    case 'any':
      return '?'
    case 'range':
      return `${st.rangeFrom}-${st.rangeTo}`
    case 'step':
      return `${st.stepFrom}/${st.stepSize}`
    case 'list':
      return st.list.length ? st.list.toSorted((a, b) => a - b).join(',') : '*'
    case 'last':
      return st.lastOffset > 0 ? `L-${st.lastOffset}` : 'L'
    case 'lastWeekday':
      return 'LW'
    case 'nearestWeekday':
      return `${st.nearestDay}W`
    case 'lastOfWeek':
      return `${st.lastDow}L`
    case 'nth':
      return `${st.nthDow}#${st.nthIndex}`
    default:
      return '*'
  }
}
...
function syncFromStates(edited?: 'day' | 'week') {
  ...
  parsing = true
  try {
    if (edited === 'day' && restricted(build(segs[3])) && states.week.mode !== 'any')
      states.week.mode = 'any'
    if (edited === 'week' && restricted(build(segs[5])) && states.day.mode !== 'any')
      states.day.mode = 'any'
  } finally {
    parsing = false
  }
...
function parseGeneric(tok: string, seg: SegDef) {
  ...
  if (tok === '?') {
    st.mode = seg.key === 'day' || seg.key === 'week' ? 'any' : 'every'
    return
  }
  ...
  if (m) {
    st.mode = 'step'
    st.stepFrom = m[1] === '*' ? seg.min : norm(+m[1])
    st.stepSize = +m[2]
    return
  }
  if (/^\d+(,\d+)*$/.test(tok)) {
    st.mode = 'list'
    st.list = tok.split(',').map(s => norm(+s))
    return
  }
...
<template>
  <div class="cron-editor">
    <n-tabs v-model:value="activeTab" type="line" size="small">
      <n-tab-pane v-for="seg in segs" :key="seg.key" :name="seg.key" :tab="seg.tab()">
        <n-radio-group
          :value="seg.st.mode"
          class="cron-rows"
          @update:value="(v: SegMode) => (seg.st.mode = v)"
        >
          <div class="cron-row">
          ...
          <div class="cron-row">
            ...
            <n-input-number
              v-model:value="seg.st.rangeFrom"
              :min="seg.min"
              :max="seg.max"
              size="small"
              class="cron-num"
              :disabled="seg.st.mode !== 'range'"
            />
            <span class="cron-sep">{{ t('job.cron.to') }}</span>
            <n-input-number
              v-model:value="seg.st.rangeTo"
              :min="seg.min"
              :max="seg.max"
              size="small"
              class="cron-num"
          ...
          <div class="cron-row">
            ...
            <n-input-number
              v-model:value="seg.st.stepFrom"
              :min="seg.min"
              :max="seg.max"
              size="small"
              class="cron-num"
              :disabled="seg.st.mode !== 'step'"
            />
            <span class="cron-sep">{{ t('job.cron.stepEvery') }}</span>
            <n-input-number
              v-model:value="seg.st.stepSize"
              :min="1"
              :max="seg.max"
              size="small"
              class="cron-num"
          ...
          <template v-if="seg.key === 'day'">
            <div class="cron-row">
              ...
              <n-input-number
                v-model:value="seg.st.lastOffset"
                :min="0"
                :max="30"
                size="small"
                class="cron-num"
                :disabled="seg.st.mode !== 'last'"
              />
            ...
            <div class="cron-row">
              ...
              <n-input-number
                v-model:value="seg.st.nearestDay"
                :min="1"
                :max="31"
                size="small"
                class="cron-num"
                :disabled="seg.st.mode !== 'nearestWeekday'"
              />
    ...
    <div v-if="expr.trim()" class="cron-preview">
      <n-alert
        v-if="previewData?.everySecondWarning"
        type="warning"
        :bordered="false"
        class="cron-warn"
      >
        ...
        <div class="cron-preview-row">
          ...
          <div class="cron-occurrences">
            <template v-if="previewData.occurrences.length">
              <div v-for="o in previewData.occurrences" :key="o" class="cron-occurrence">
                {{ o.replace('T', ' ') }}
              </div>
            </template>
