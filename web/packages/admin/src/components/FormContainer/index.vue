...
const props = defineProps<{
  ...
  /**
   * 铺满视口。默认 `false` —— 不给 `'auto'` 当默认值是刻意的:没传这个 prop 的既有页面
   * 不该因为换了台平板就换一种形态。字段多的表单显式传 `true`,想跟着屏幕走再传 `'auto'`
   * (紧凑屏判定见 composables/useCompactScreen)。
   */
...
const fullscreen = computed(() =>
  props.fullscreen === 'auto' ? compact.value : props.fullscreen === true,
)
const w = computed(() =>
  fullscreen.value ? winW.value : Math.min(props.width ?? 560, Math.round(winW.value * 0.9)),
)
// 全屏时不再钉宽度,交给 .smart-form--fullscreen 那条全局样式铺满。
const modalStyle = computed(() => (fullscreen.value ? undefined : { width: `${w.value}px` }))
...
<template>
  ...
  >
    ...
    <template #footer>
      <slot name="footer">
        <n-space justify="end">
          <n-button :disabled="loading" @click="show = false">
...
<!-- 不能加 scoped:n-modal / n-drawer 的内容 teleport 到 body,scoped 编译出的 data-v 属性到不了那棵子树,
     写了等于没写(这正是"全屏样式不生效"最常见的原因)。故用带前缀的类名自行隔离。 -->
<style>
.smart-form--fullscreen {
  width: 100vw;
  max-width: 100vw;
  height: 100dvh;
  max-height: 100dvh;
