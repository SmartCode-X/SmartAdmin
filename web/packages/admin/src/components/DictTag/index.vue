...
// 字典值 → 标签(表格列翻译)。SysDictItem 暂无样式字段,配色靠 fallback 约定。
// ponytail: 启用项按 sort 升序取下标 i%4 映射固定语义色序(info/success/warning/error),
// 稳定性由后端 sort 保证——即后端通过排序间接控制颜色;后端加 tagType 字段后,这里改读字段并废弃本约定。
...
<template>
  <!-- 空值按页面惯例渲染「—」,不渲染空标签 -->
  <n-tag v-if="value != null && value !== ''" :type="type" size="small" :bordered="false">
    {{ label }}
  </n-tag>
  <span v-else>—</span>
</template>
