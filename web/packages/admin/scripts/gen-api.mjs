...
// 后端端口不是常量。dev 代理用 SMART_API_TARGET 指到别处(见 web/template/vite.config.ts,
// 仓库根的 dev-start.sh 也这么覆盖),gen:api 若写死 http://localhost:5100,后端跑在
// 别的端口时这条命令要么连不上,要么把「另一个后端」的契约静默写进 schema.d.ts。
// npm script 里没有跨平台的环境变量展开写法($VAR 在 cmd.exe 不展开),所以和 gen:icons
// 一样走一个 node 脚本,让两处读同一个变量。
//
// 用法:
...
// 与 web/template/vite.config.ts 的 apiTarget 同一个变量、同一个默认值(5100 而非 5000:macOS 的 AirPlay 占了 5000)。
