 ...
 * 每个用例自己把前置条件立起来,用例之间就没有顺序依赖。
 */

export const ADMIN_ACCOUNT = process.env.SMART_E2E_ACCOUNT ?? 'superAdmin'
export const ADMIN_PASSWORD = process.env.SMART_E2E_PASSWORD ?? 'Aa123456'
 ...
 *
 * 前置:调用者须有 **2 个以上**应用。那颗按钮由 `packages/admin/src/layouts/AppHeader.vue` 的 `auth.modules.length > 1` 把关,
 ...
 * `AppHeader.vue` 里有三个 `mode="horizontal"` 的 `n-menu`——横向菜单的目录**不带箭头**,
 ...
 * 种子里「文件管理」既是目录(Id 600)又是它下面的叶子(Id 610),子串匹配 + `.first()` 按文档序命中的是目录,
