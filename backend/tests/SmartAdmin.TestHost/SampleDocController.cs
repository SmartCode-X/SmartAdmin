    ...
    IExcelWriter writer) : ControllerBase
{
    ...
    /// <summary>
    /// 只挂 [Authorize]、不挂任何内核授权特性的端点——消费者自建控制器最容易写成这样。
    /// 数据范围由全局自动绑定给出真实范围,而不是回退到"不受限"(否则这里就能看到全机构的行)。
    /// </summary>
    [HttpGet("authorize-only")]
    [Authorize]
    public async Task<Result<IReadOnlyList<SampleDoc>>> ListAuthorizeOnly() =>
        Result<IReadOnlyList<SampleDoc>>.Ok(await svc.ListAsync());
