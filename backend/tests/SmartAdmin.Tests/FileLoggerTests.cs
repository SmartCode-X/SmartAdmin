...
{
    private readonly string _root = Path.Combine(Path.GetTempPath(), "smart-filelog-" + Guid.NewGuid().ToString("N")[..8]);
    ...
    public void 跨天换新文件_且不截断昨天的()
    {
        using (var provider = Create())
        {
            ...
            WaitUntil(() => FileContains(LogFile("error", "20260714"), "今天的日志"));   // 消费任务落盘后再推时钟

            _time.Advance(TimeSpan.FromDays(1));
            logger.LogError("明天的日志");
        }

        // 有些同类实现换文件时 SetLength(0),会把已存在的日期文件清零。这里必须两天都在
    ...
    public void 目录不可写时不抛异常_降级为不写盘()
    {
        ...
        var blocker = Path.Combine(Path.GetTempPath(), "smart-filelog-blocker-" + Guid.NewGuid().ToString("N")[..8]);
        ...
        try
        {
            ...
            // 日志系统绝不能打崩宿主:写不进去就写不进去,业务调用方毫无感知。Dispose 排干队列时才真正尝试写盘
    ...
    /// <summary>
    /// 队列是异步消费的:Dispose 会排干,但用例中途要看文件/推时钟就得等消费任务追上来。
    /// 不能固定 sleep:忙的 CI 机器上 300ms 追不上,时钟先被推到第二天,两天的日志全写进第二天的文件。
    /// </summary>
    private static void WaitUntil(Func<bool> condition)
    {
        var deadline = DateTime.UtcNow + TimeSpan.FromSeconds(10);
        while (!condition() && DateTime.UtcNow < deadline) Thread.Sleep(20);
        Assert.True(condition(), "消费任务 10 秒内没有把日志落盘");
    }

    /// <summary>
    /// 写入方还开着文件,读取方必须也允许对方的写共享(File.ReadAllText 默认只共享读,Windows 上会共享冲突)。
    /// 文件还没建出来就当没写。
    /// </summary>
...
public class FileLoggerSetupTests
{
    ...
    {
        var dir = Path.Combine(Path.GetTempPath(), "smart-filelog-setup-" + Guid.NewGuid().ToString("N")[..8]);
        try
        {
            using var f = Factory(
                ("SmartAdmin:Logging:File:Enabled", "true"),
                ("SmartAdmin:Logging:File:Path", dir));       // 绝对路径:别在 TestHost 目录里拉屎
    ...
    /// <c>IWebHostEnvironment.WebRootPath</c> 在 wwwroot 目录不存在时是 <c>null</c>,守卫若直接信它,
    /// 就会在<b>全新部署的机器上恰好失灵</b>(那儿 wwwroot 还没有),等第一次上传把 wwwroot 建出来,
    /// 日志目录就被静态中间件直出了;本用例也会随"别的测试跑没跑过上传"随机红绿。</para>
    /// </summary>
    [Fact]
    public void 日志目录落在wwwroot下_启动即抛()
    {
        using var f = Factory(
            ("SmartAdmin:Logging:File:Enabled", "true"),
            ("SmartAdmin:Logging:File:Path", "wwwroot/logs"));
