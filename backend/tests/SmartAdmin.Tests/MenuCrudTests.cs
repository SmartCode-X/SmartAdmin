...
/// <summary>
/// 菜单管理 CRUD 的 HTTP 级回归。业务失败走统一信封 HTTP 200 + 业务码
...
public class MenuCrudTests
{
    ...
    /// <summary>
    /// 无页面权限项是一等公民:按钮直接挂在目录下(目录不填 Path / Component),不必建假页面当容器。
    /// 锁三件事:后端接受这种节点;授给角色后 [RolePermission] 照常放行;门户菜单树对它零感知。
    /// 这条防的是后人"好心"给菜单加"按钮必须挂页面下 / Path 必填"的校验。
    /// </summary>
    [Fact]
    public async Task Button_under_catalog_without_page_grants_permission()
    {
        using var f = new AdminAppFactory();
        var admin = await SuperAdminClient(f);

        // 一个只作权限组的目录(无 Path / Component)+ 一颗只给机器调的接口按钮
        var groupId = await AddMenu(admin, new { parentId = 0, type = 1, title = "移动端接口", permission = "", sort = 99, enabled = true, moduleId = 1, visible = false });
        var buttonId = await AddMenu(admin, new { parentId = groupId, type = 3, title = "探针", permission = "GET:/api/v1/ping", sort = 1, enabled = true });
        ...
        var pda = f.CreateClient();
        pda.DefaultRequestHeaders.Authorization = new AuthenticationHeaderValue("Bearer", await pda.LoginToken("pda", "Test@123456"));

        // 只授了目录下的按钮 → 该码放行,没授的码 403
        Assert.Equal(System.Net.HttpStatusCode.OK, (await pda.GetAsync("/api/v1/ping")).StatusCode);
        Assert.Equal(System.Net.HttpStatusCode.Forbidden, (await pda.GetAsync("/api/v1/sys/user/page?Current=1&Size=10")).StatusCode);

        // 门户菜单树:目录作为祖先脚手架保留,但按钮不进树、目录没有任何可导航子项
        var portal = (await (await pda.GetAsync("/api/v1/personal/menu?moduleId=1")).ReadEnvelope()).GetProperty("data");
        var group = Find(portal, groupId);
        Assert.NotNull(group);
        Assert.Empty(group.Value.GetProperty("children").EnumerateArray());
        Assert.Null(Find(portal, buttonId));

        // 管理端全量树仍能看到这颗按钮(角色授权界面靠它渲染「接口权限(无页面)」一行)
        var adminTree = await TreeData(admin);
        var btn = Find(adminTree, buttonId);
        Assert.NotNull(btn);
        Assert.Equal(groupId, btn.Value.GetProperty("parentId").GetInt64());
    }
