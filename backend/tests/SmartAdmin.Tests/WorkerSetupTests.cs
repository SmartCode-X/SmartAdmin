...
/// <summary>
/// 独立 Worker 的组合根:装配得出调度器、WorkerId 守卫、租约参数守卫。
...
public class WorkerSetupTests
{
    ...
    private static Dictionary<string, string?> Baseline(string dbFile) => new()
    {
        ...
        ["SmartAdmin:Database:ConnectionString"] = $"Data Source={dbFile}",
        ["SmartAdmin:Database:EnableCodeFirst"] = "false",   // schema 归 API 侧所有
        ["SmartAdmin:Database:EnableSeed"] = "false",
        ["SmartAdmin:Id:WorkerId"] = "7",
    };

    /// <summary>照 samples/WorkerHost 的真实姿势装配:Generic Host + 一行 AddSmartAdminWorker。</summary>
    [Fact]
    public void Worker_host_resolves_the_scheduler_and_its_dependencies()
    {
        var dbFile = Path.Combine(Path.GetTempPath(), $"smart-worker-{Guid.NewGuid():N}.db");
    ...
    public void Worker_without_explicit_worker_id_throws()
    {
        ...
        settings.Remove("SmartAdmin:Id:WorkerId");
        var services = new ServiceCollection();
        services.AddLogging();

        var ex = Assert.Throws<InvalidOperationException>(() => services.AddSmartAdminWorker(Config(settings)));
    ...
    {
        var settings = Baseline("ignored.db");
        settings["SmartAdmin:Jobs:HeartbeatSeconds"] = "10";
        settings["SmartAdmin:Jobs:LeaseSeconds"] = "20";   // 必须 > 2×心跳,否则一次 GC 停顿就丢主、主备震荡
        var services = new ServiceCollection();
        services.AddLogging();
    ...
    public void Worker_with_invalid_blocked_cidr_throws()
    {
        ...
        var settings = Baseline("ignored.db");
        settings["SmartAdmin:Jobs:Http:BlockedCidrs:0"] = "not-a-cidr";
        var services = new ServiceCollection();
        services.AddLogging();
    ...
    {
        var settings = Baseline("ignored.db");
        settings[$"SmartAdmin:Jobs:{key}"] = value;
        var services = new ServiceCollection();
        services.AddLogging();
    ...
    {
        var settings = Baseline("ignored.db");
        settings["SmartAdmin:Jobs:Http:MaxResponseLogBytes"] = "-1";
        var services = new ServiceCollection();
        services.AddLogging();
    ...
    public void Worker_entity_scan_includes_services_assembly()
    {
        ...
        using var host = builder.Build();
        var sources = host.Services.GetRequiredService<SmartEntitySources>();
