...
/// <summary>
/// 定时任务的安全面回归(安全审查抓出的实证问题逐条钉死)。
/// 每条都对应一份真实出现过的绕过手法,删掉对应防线本类必红。
/// </summary>
public class JobSecurityTests
{
    ...
    // 回环:任务打宿主自己的管理端口不是用例,却是一条现成的绕过网络边界的通道
    ...
    [InlineData("169.254.169.254", "169.254.169.254", true)]     // 无斜杠 = 单地址,应正确匹配,不静默失效
    ...
    public async Task Compiled_kind_cannot_impersonate_built_in_handlers()
    {
        ...
        var http = await (await admin.PostJson("/api/v1/sys/job", new
        {
            ...
            name = "冒充 HTTP",
            handlerKind = 1,
            handlerName = "SmartAdmin.Services.HttpAdminJob",
            triggerKind = 1,
            cronExpression = "0 30 3 * * ?",
        ...
        var sql = await (await admin.PostJson("/api/v1/sys/job", new
        {
            ...
            name = "冒充 SQL",
            handlerKind = 1,
            handlerName = "SmartAdmin.Services.SqlAdminJob",
            triggerKind = 1,
            cronExpression = "0 30 3 * * ?",
