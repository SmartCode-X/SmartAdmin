...
/// <summary>
/// 定时任务 HTTP 端点回归:CRUD、启停、执行一次、cron 预览、处理器清单、
...
public class JobApiTests
{
    ...
    public async Task Existing_sql_job_can_update_non_payload_while_gate_closed()
    {
        using var f = new AdminAppFactory
        {
            Settings = new Dictionary<string, string?> { ["SmartAdmin:Jobs:Sql:Enabled"] = "true" },
        };
        var admin = await SuperAdminClient(f);
    ...
    public async Task Handlers_endpoint_lists_built_in_handlers()
    {
        ...
        Assert.Contains("SmartAdmin.Services.JobLogCleanupJob", names);
        Assert.Contains("SmartAdmin.Services.HttpAdminJob", names);
        Assert.Contains("SmartAdmin.Services.SqlAdminJob", names);
    ...
    public async Task Deleted_job_is_restored_as_paused()
    {
        ...
        Assert.Equal(2, restored.GetProperty("status").GetInt32());            // 强制 Paused
    ...
    {
        // 任务能执行任意 HTTP/SQL,演示站绝不放行——这是特性不是缺陷
        using var f = new AdminAppFactory { Settings = new Dictionary<string, string?> { ["SmartAdmin:DemoMode"] = "true" } };
        var admin = await SuperAdminClient(f);
