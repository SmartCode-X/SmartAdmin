...
/// <summary>
/// 多副本共用同一个雪花机器号 = 撞主键。这是<b>数据损坏级</b>的问题,
...
public class WorkerIdGuardTests
{
    ...
    public void Redis_cache_without_an_explicit_worker_id_fails_fast()
    {
        using var f = Factory(
            ...
            ("SmartAdmin:Cache:RedisConnectionString", "localhost:6379"));
    ...
    [Fact]
    public void Memory_cache_without_a_worker_id_auto_leases_one()
    {
        // 零配置单实例:不配 WorkerId 照常跑,零配置启动不受影响。但并非"回落 0":
        // 启动时用文件锁抢一个本机空闲槽位并回写到 AdminIdOptions,发号器、任务节点名、日志后缀读到的都是它。
        using var f = new AdminAppFactory();

        _ = f.CreateClient();
        var leased = f.Services.GetRequiredService<AdminIdOptions>().WorkerId;
        Assert.NotNull(leased);
        Assert.InRange(leased.Value, 0, (int)SnowflakeIdGenerator.MaxWorkerId);

        var id = f.Services.GetRequiredService<IIdGenerator>().NextId();
        Assert.Equal(leased.Value, (int)((id >> 6) & 63));   // 发号器用的就是抢到的这个号
    }

    [Fact]
    public void Two_hosts_on_one_machine_never_share_a_worker_id()
    {
        // 模拟 IIS 应用池重叠回收 / Web 园 / 同机多份部署:两个进程读同一份配置,若都回落到 0,同毫秒即撞主键。
        // 同进程内两个宿主与两个进程在文件锁语义上等价(按打开句柄互斥),故足以复现。
        using var a = new AdminAppFactory();
        using var b = new AdminAppFactory();
        _ = a.CreateClient();
        _ = b.CreateClient();

        Assert.NotEqual(
            a.Services.GetRequiredService<AdminIdOptions>().WorkerId,
            b.Services.GetRequiredService<AdminIdOptions>().WorkerId);
    }
}
