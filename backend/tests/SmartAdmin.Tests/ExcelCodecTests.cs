...
/// <summary>
/// 可选包 codec 测试:DI 前置替换 / 默认 Missing 抛 46001 / 模板真出 dataValidation。
...
public class ExcelCodecTests
{
    ...
    /// <summary>
    /// 不调 <c>AddSmartAdminExcel()</c> 时,内核 TryAdd 的 <see cref="MissingExcelProvider"/> 生效;
    ...
    /// <summary>
    /// 在内核 TryAdd <b>之前</b>调 <c>AddSmartAdminExcel()</c> → 解析到真实现(TryAdd 前置替换)。
    /// 变异:把 ExcelSetup 里的 TryAdd 注释掉或改到之后注册 → 本条必须红。
    /// </summary>
    [Fact]
    public void AddSmartAdminExcel_BeforeKernel_WinsTryAdd()
    {
        var services = new ServiceCollection();
        services.AddSmartAdminExcel(); // 前置
        services.TryAddSingleton<IExcelReader, MissingExcelProvider>();
        services.TryAddSingleton<IExcelWriter, MissingExcelProvider>();
        services.TryAddSingleton<IExcelTemplateBuilder, MissingExcelProvider>();
