...
/// <summary>
/// 请求限流回归。认证端点(/api/v1/auth/*)按 IP 更严一档,超阈返回 429 + 统一信封(40008);
...
public class RateLimitTests
{
    ...
    // 断言 429 的用例必须传 clock:限流键含 `nowSeconds / window`(固定窗口,见 RateLimitMiddleware),
    // 真实时钟下连打的几次请求一旦跨过窗口边界,计数就归零、本该被限的那次反而放行。
    // 生产行为是有意的(边界允许 2× 突发),所以修测试而不是修中间件。只断言 OK 的用例不受影响,不必冻结。
    private static AdminAppFactory Enabled(TimeProvider? clock = null) =>
        new()
        {
            Settings = new Dictionary<string, string?> { ["SmartAdmin:Security:RateLimit:Enabled"] = "true" },
            Overrides = clock is null
                ? null
                : s =>
                {
    ...
    {
        using var f = Enabled(new FakeTimeProvider(DateTimeOffset.UtcNow));   // 三次请求必落同一窗口
    ...
    public async Task Runtime_toggle_off_via_config_stops_limiting()
    {
        ...
        using var f = Enabled(new FakeTimeProvider(DateTimeOffset.UtcNow));   // 前两次请求必落同一窗口
