...
/// <summary>
/// <see cref="SmartHub.OnConnectedAsync"/> 必须在入组前核验 <c>sid</c> 仍活跃——
...
public class SmartHubTests
{
    ...
    public async Task Missing_sid_aborts_and_joins_no_group()
    {
        ...
        var hub = new SmartHub(new FakeSessionService(active: true)) { Context = ctx, Groups = groups };
    ...
    public async Task Inactive_sid_aborts_and_joins_no_group()
    {
        ...
        var hub = new SmartHub(new FakeSessionService(active: false)) { Context = ctx, Groups = groups };
