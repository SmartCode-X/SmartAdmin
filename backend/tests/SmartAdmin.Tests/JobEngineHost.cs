...
internal sealed class JobEngineHost : IAsyncDisposable
{
    ...
    public JobEngineHost(
        ...
        int workerId = 0)   // 多宿主共库必须互异——同号同毫秒雪花撞主键
    {
        Clock = clock;
    ...
    public void InitTables()
    {
        ...
        // Panic 站内信定向到超管 Id=1(校验目标必须存在);精简宿主不跑种子,这里补一行。
    ...
    /// <summary>
    /// 本机 kill,失败就重试到超时:执行器先插 Running 行、解析完处理器才登记可取消,
    /// 所以刚查到的行可能还没登记(慢库 + 线程池忙时窗口有几十毫秒),直接调一次会误判 false。
    /// </summary>
    public async Task<bool> CancelLocalAsync(long logId, int timeoutMs = 5000)
    {
        var deadline = Environment.TickCount64 + timeoutMs;
        while (!Executor.TryCancelLocal(logId))
        {
...
/// <summary>暴露 protected 步骤(领取 CAS)+ 顺带证明子类可继承(同 ReplaceabilityTests 的前置注册姿势)。</summary>
