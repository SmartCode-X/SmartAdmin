...
/// <summary>本地存储路径穿越防护。IDisposable 清理临时根。</summary>
public class LocalFileStorageTests : IDisposable
{
    private readonly string _root = Path.Combine(Path.GetTempPath(), "smart-test-" + Guid.NewGuid().ToString("N")[..8]);
    ...
    {
        var path = await _storage.SaveAsync(Bytes("hello-smart"), "20260707/abc.txt");
        ...
        {
            Assert.NotNull(read);
            Assert.Equal("hello-smart", await new StreamReader(read!).ReadToEndAsync());
        }
