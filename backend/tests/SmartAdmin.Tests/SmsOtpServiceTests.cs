...
{
    private static (AdminAppFactory f, MfaLoginFlowTests.CapturingSmsSender sms, FakeTimeProvider clock) Factory()
    {
        var sms = new MfaLoginFlowTests.CapturingSmsSender();
        // 日发送上限按本地日分桶,测试要算出同一个桶键——共用冻结时钟,别各读各的墙上时间
        var clock = new FakeTimeProvider(DateTimeOffset.UtcNow);
        var f = new AdminAppFactory
        {
            Overrides = s =>
            {
                s.Replace(ServiceDescriptor.Singleton<ISmsSender>(sms));
                s.Replace(ServiceDescriptor.Singleton<TimeProvider>(clock));
            },
        };
        return (f, sms, clock);
    }
    ...
    public async Task Daily_send_limit_is_enforced_per_phone()
    {
        ...
        var day = clock.GetLocalNow().ToString("yyyyMMdd");
