...
/// <summary>
/// 操作日志覆盖面回归锁:过滤器是 opt-out 的——<b>写操作默认留痕</b>,不依赖有人记得挂 [OperationLog]。
/// <para>opt-in(只记挂了特性的动作)会让角色授权/数据范围/系统配置/停用用户/强制下线这些最该审计的动作
/// 全部无痕,而分片上传每片都留痕。本测试锁死"改回 opt-in 即变红"。</para>
/// </summary>
public class OperationLogCoverageTests
{
    ...
    public async Task Write_endpoints_without_marker_are_logged_with_route_as_title()
    {
        ...
        // 建个角色,再给它设数据范围 —— 两个端点都没挂 [OperationLog],靠 opt-out 默认规则仍应留痕
