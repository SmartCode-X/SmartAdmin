...
/// <summary>
/// 宿主级横切端点回归:健康检查存活/就绪、CORS 命名策略、OpenAPI 文档内容与模块禁用一致性。
/// </summary>
public class HostEndpointsTests
{
    ...
    {
        // 配置放行源后,带 Origin 的请求得到 Access-Control-Allow-Origin(经 IStartupFilter 挂载的全局命名策略)
        using var f = new AdminAppFactory
        {
            Settings = new Dictionary<string, string?> { ["SmartAdmin:Api:Cors:AllowedOrigins:0"] = "https://admin.example.com" },
        };
        var c = f.CreateClient();
    ...
    {
        // 契约源:开发环境暴露 /openapi/v1.json;含统一信封字段;被禁模块(Upload)的路由缺席
