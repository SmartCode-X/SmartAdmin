...
public class MultiConfigIdTests
{
        ...
        Action<IServiceCollection>? configure = null)
    {
        ...
        var mainFile = Path.Combine(Path.GetTempPath(), $"smart-{id}-main.db");
        var sideFile = Path.Combine(Path.GetTempPath(), $"smart-{id}-side.db");
    ...
    public async Task Secondary_OptInAuditAop_FillsSnowflakeId()
    {
        ...
        await using (sp)
        {
            ...
            Assert.True(stored.Id >= SmartSeedIds.SnowflakeFloor,
    ...
    public void Secondary_OnError_LogsFailedSql()
    {
        ...
        {
            var side = sp.GetRequiredService<ISqlSugarClient>().AsTenant().GetConnection(SideConfigId);
            // 触发主连接 action → 为副库挂 OnError(每 client 实例一次)
            _ = sp.GetRequiredService<ISqlSugarClient>().CurrentConnectionConfig;

            Assert.ThrowsAny<Exception>(() =>
                side.Ado.ExecuteCommand("SELECT 1 FROM smart_mc_no_such_table_side"));

            Assert.Contains(log.Entries, e =>
                e.Level == LogLevel.Error
                && e.Text.Contains("smart_mc_no_such_table_side", StringComparison.Ordinal));
    ...
    /// <summary>
    /// 回归:内核不得 QueryFilter.Clear / 清空 AOP 覆盖消费方钩子。
    ...
    public void Secondary_PreservesConsumerOnError_AfterRepeatedGetConnection()
    {
        ...
        using (sp)
        {
            ...
            // 再触主库 + 再 Get 副库:若实现每次 Clear/重挂,就会把 OnError 改回内核 logger
            db.Ado.GetInt("SELECT 1");
            var side2 = db.AsTenant().GetConnection(SideConfigId);
            // 同实例时 side2 已是 custom;若被内核重挂则 customHits 不会增加
            Assert.ThrowsAny<Exception>(() =>
                side2.Ado.ExecuteCommand("SELECT 1 FROM smart_mc_consumer_onerror_probe"));
    ...
    public void Secondary_Sqlite_EnsuresParentDirectory()
    {
        ...
        var nest = Path.Combine(Path.GetTempPath(), $"smart-mc-nest-{id}", "deeper");
        var mainFile = Path.Combine(Path.GetTempPath(), $"smart-{id}-main.db");
    ...
    public void Secondary_RelativeSqlitePath_ResolvesAgainstContentRoot()
    {
        ...
        var contentRoot = Path.Combine(Path.GetTempPath(), $"smart-mc-cr-{Guid.NewGuid():N}");
    ...
    /// <summary>
    /// 并发场景:多线程 + SuppressFlow 同时取副库并查询。
    ...
    public void ValidateAdditionalDatabases_RejectsInvalidEntries()
    {
        ...
        // DbType default is now empty → omitting it must be caught
    ...
    public void SingleDatabase_StillRegistersOnlyMainConfig()
    {
        ...
        var mainFile = Path.Combine(Path.GetTempPath(), $"smart-{id}.db");
