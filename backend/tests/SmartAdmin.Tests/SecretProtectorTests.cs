...
public class SecretProtectorTests
{
    ...
    /// <summary>非开发环境启用 TOTP 却没配主密钥:自动开发密钥重启即失效,宁可启动就拒。</summary>
    [Fact]
    public void Totp_outside_development_without_configured_key_throws()
    {
        var security = new AdminSecurityOptions
        {
            Totp = new AdminTotpOptions { Enabled = true },
            DataProtection = new AdminDataProtectionOptions { Key = null },
        };
        Assert.Throws<InvalidOperationException>(() =>
            new LocalDataProtectionKeyProvider(security, contentRoot: Path.GetTempPath(), isDevelopment: false));
    }
