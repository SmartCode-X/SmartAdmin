...
/// <summary>
/// 可授权路由清单:把"权限码"从管理员手敲的魔法字符串,变成从后端真实路由表里选。
...
public class PermissionRoutesEndpointTests
{
    ...
    public async Task Codes_from_the_list_actually_grant_access()
    {
        ...
        // 找到承载该权限码的菜单按钮(种子里的「用户-查询」,它挂着列表与详情两条码),授给角色
    ...
    /// <summary>在菜单树里找承载该权限码的节点 Id(深度优先;一颗按钮可挂多条码,按 Split 展开比对)。</summary>
    private static long? FindByPermission(System.Text.Json.JsonElement nodes, string permission)
    {
        foreach (var n in nodes.EnumerateArray())
        {
            if (n.TryGetProperty("permission", out var p) && PermissionCode.Split(p.GetString()).Contains(permission))
