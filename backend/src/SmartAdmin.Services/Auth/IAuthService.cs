...
/// <summary>
/// 认证服务。默认实现 <see cref="AuthService"/> 是"模板方法可覆写"
/// 的招牌样板——登录长流程拆成小步,每步 protected virtual,
...
public interface IAuthService
{
    ...
    /// <summary>
    /// 外部登录(OAuth/SSO 回调):用授权码换外部身份 → 找绑定 →(未绑定按 provider 策略 拒绝/开户)→
    ...
    /// <summary>用刷新令牌换发新令牌对(轮换 + 复用检测);失败抛 40007。</summary>
