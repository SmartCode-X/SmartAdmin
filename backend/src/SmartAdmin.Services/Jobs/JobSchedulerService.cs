...
/// <summary>
/// 调度循环:心跳/选主 + 主节点扫表派发,deadline 睡眠。
/// <para><b>正确性分工</b>:<c>sys_job_lock</c> 租约只回答「谁来扫表」(效率);
...
/// <para>唤醒三通道:本进程改任务 → <see cref="JobChangedEvent"/> 即刻唤醒;别的副本改任务 →
    ...
    ILogger<JobSchedulerService> logger) : BackgroundService
{
    ...
    protected override async Task ExecuteAsync(CancellationToken stoppingToken)
    {
        ...
        {
            logger.LogInformation("SmartAdmin 调度器:本副本不参与调度(Jobs:SchedulerEnabled=false);执行一次/查询/编辑照常。");
            return;
        }
        ...
        while (!stoppingToken.IsCancellationRequested)
        {
            ...
            catch (Exception ex)
            {
                ...
                logger.LogWarning(ex, "SmartAdmin 调度器:本拍失败,下拍重试。");
            }
    ...
    public override async Task StopAsync(CancellationToken cancellationToken)
    {
        ...
        await executor.DrainAsync(cancellationToken);   // 宽限期内 drain,逾期逐个 Cancel
    }
    ...
    /// <summary>锁行幂等 ensure-insert + SQLite 集群警告。</summary>
    protected virtual async Task EnsureInitializedAsync(DateTime now, CancellationToken cancellationToken)
    {
        ...
        if (string.Equals(dbOptions.DbType, "Sqlite", StringComparison.OrdinalIgnoreCase))
        {
            ...
            var others = await db.Queryable<SysJobNode>()
                ...
                .CountAsync();
            if (others > 0)
                logger.LogWarning("SQLite 下检测到第二个活跃调度节点——SQLite 不支持集群形态(锁争用/损坏风险),集群/Worker 请改用服务器数据库。");
        }
        _initialized = true;
    }
    ...
    protected virtual async Task<int> ReapOrphanRunsAsync(DateTime now, CancellationToken cancellationToken)
    {
        ...
        var reaped = await db.Updateable<SysJobLog>()
            ...
            .ExecuteCommandAsync();
        if (reaped > 0)
            logger.LogWarning("SmartAdmin 调度器:回收 {Count} 条失联/更替实例遗留的未闭合执行记录。", reaped);
        return reaped;
    }

    private static string AliveKey(string nodeName, string? instanceId) => $"{nodeName}\0{instanceId ?? ""}";

    /// <summary>心跳:节点行 upsert + 主节点续租 / 备节点夺租。全部参数化 UPDATE 按影响行数判定,四库通吃。</summary>
    protected virtual async Task HeartbeatAsync(DateTime now, CancellationToken cancellationToken)
    {
        ...
        if (_isLeader)
        {
            ...
            if (renewed != 1)
            {
                ...
                logger.LogWarning("SmartAdmin 调度器:续租失败,节点 {Node} 退回备节点。", NodeName);
            }
            return;
        }
        ...
        if (acquired == 1)
        {
            _isLeader = true;
            _dirty = true;   // 新主全量重载
            logger.LogInformation("SmartAdmin 调度器:节点 {Node} 当选主节点。", NodeName);
        }
    }
    ...
    {
        _nodeStartTime ??= now;
        // SqlSugar SetColumns 表达式会求值闭包成员;idOptions.WorkerId 为 null 时 ?? 会炸。
        ...
        foreach (var job in rows)
        {
            if (job.NextRunTime is null)
            {
                ...
                else
                {
                    ...
                    // 少了这半句会把用户刚救活的任务判死,而且留下 Completed + 非空 NextRunTime 违反状态机不变量
    ...
    /// <summary>派发到期任务:限流 → 错过判定 → 领取 CAS → 触发/记账。</summary>
    ...
    /// <summary>
    /// 领取 CAS——防双发的<b>唯一</b>正确性来源:影响行数 = 1 才允许触发。
    /// <c>NextRunTime = @expected</c> 那半句是命根子,删掉它双发测试必须红。
    /// </summary>
    ...
    /// <summary>Skip 策略:错过合并记一行 MissedSkipped(记次数,不刷表)。</summary>
    ...
    /// <summary>SerialSkip:上次触发未结束,本次跳过并记 Skipped 行(丢弃可见)。</summary>
