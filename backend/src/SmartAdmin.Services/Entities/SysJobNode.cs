...
/// <summary>
/// 调度节点注册表——每个参与调度的进程(含备节点、Worker)每次心跳 upsert 自己一行。
/// <para><b>不落 IsLeader 列</b>:谁是主由查询时与 <see cref="SysJobLock"/> 行比对得出,避免双写不一致。
/// 心跳超 24h 的陈尸行由 JobLogCleanupJob 顺手清。Id 由代码显式赋雪花(PrimaryId 不种子化)。</para>
/// </summary>
...
public class SysJobNode : PrimaryId
{
    ...
    /// <summary>雪花机器号(SmartAdmin:Id:WorkerId;监控页展示,排查同号碰撞)</summary>
