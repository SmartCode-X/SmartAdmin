...
/// <summary>
/// 定时任务表——一行完整声明一个任务:触发配置 + 载荷 + 失败策略 + 运行状态。
...
/// <para><see cref="NextRunTime"/> 是<b>领取列</b>:每次触发前对它做原子 CAS,防双发的唯一正确性来源;
/// 所有写入路径必须先整秒截断(MySQL datetime 毫秒四舍五入会让 CAS 无声失效)。</para>
/// </summary>
...
public class SysJob : BaseEntity
{
    ...
    /// <summary>属性包:Dictionary&lt;string,string?&gt; JSON,处理器参数的唯一入口</summary>
    ...
    /// <summary>6 段秒级 cron(入库前归一化 + 统一大写)</summary>
    [SugarColumn(Length = 64, IsNullable = true, ColumnDescription = "cron 表达式(归一化 6 段)")]
    public string? CronExpression { get; set; }

    /// <summary>固定间隔秒数,≥5(&lt;5 拒 47004,防日志表爆炸)</summary>
