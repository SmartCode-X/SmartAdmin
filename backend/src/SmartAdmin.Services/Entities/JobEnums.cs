...
/// <summary>任务载荷类型(<c>sys_job.HandlerKind</c>)</summary>
public enum JobHandlerKind
{
    /// <summary>编译类:IAdminJob 实现,HandlerName 按 Name 匹配(GET /handlers 下拉可选)</summary>
    Compiled = 1,

    /// <summary>HTTP 请求:属性包给 url/method/headers/body;发出前经 SSRF 围栏(协议/主机/CIDR 校验)</summary>
    Http = 2,

    /// <summary>SQL 语句:属性包给 sql;总闸 Jobs:Sql:Enabled 默认关闭</summary>
    Sql = 3,
}
...
{
    /// <summary>6 段秒级 cron(入库前归一化)</summary>
    Cron = 1,
...
/// <summary>并发模式:上一次触发未结束时,本次怎么办(刻意不做「排队」)</summary>
...
/// <summary>
/// 任务状态(<c>sys_job.Status</c>)。<b>刻意没有 Running 态</b>——「正在运行」由未闭合的
