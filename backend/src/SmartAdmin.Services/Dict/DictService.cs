    ...
    IEventBus events) : IDictService
{
    ...
    {
        // 查重纳入软删行:唯一索引覆盖已软删行,漏检会撞库唯一约束抛原生 500
    ...
    {
        var entity = await GetTypeAsync(id);
        AdminException.ThrowIf(entity.Id <= SmartSeedIds.KernelMax, ErrorCode.SeedDataProtected);
        await types.Db.RunInTransactionAsync(async () =>
        {
    ...
    public virtual async Task DeleteTypesBatchAsync(IReadOnlyCollection<long> ids)
    {
        ...
        {
            AdminException.ThrowIf(t.Id <= SmartSeedIds.KernelMax, ErrorCode.SeedDataProtected);
        }
        // 类型行与它名下的项必须一起没、一起留:半途失败会留下"类型删了、项还在"的孤儿数据
        await types.Db.RunInTransactionAsync(async () =>
        {
            foreach (var t in targets)
            {
    ...
    public virtual async Task DeleteItemsBatchAsync(IReadOnlyCollection<long> ids)
    {
        ...
        foreach (var it in targets)
            AdminException.ThrowIf(it.Id <= SmartSeedIds.KernelMax, ErrorCode.SeedDataProtected);
        // 包一个事务:删到一半抛异常时,调用方看到的是一个失败却删掉了一部分的批量操作
        await items.Db.RunInTransactionAsync(async () =>
        {
    ...
    /// <inheritdoc />
    public virtual async Task InvalidateAsync(string typeCode)
    {
