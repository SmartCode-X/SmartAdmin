...
public class SmsOtpService(
    ...
    // 可选尾参 TimeProvider:DI 正常注入,消费者子类不传也能编译;本地日桶(EnsureSendAllowedAsync)走注入时钟,便于测试固定日期
    TimeProvider? time = null) : ISmsOtpService
{
    ...
    public virtual async Task VerifyAsync(string purpose, string cacheKey, string code)
    {
        ...
        // 原子取删:并发携同一码只有一个调用取到非空值,保证验证码一次性消费
    ...
    protected virtual async Task EnsureSendAllowedAsync(string phone)
    {
        ...
        {
            var day = (time ?? TimeProvider.System).GetLocalNow().ToString("yyyyMMdd");   // 本地日桶,走注入时钟便于测试固定日期
