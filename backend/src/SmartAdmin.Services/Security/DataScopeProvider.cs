    ...
    AdminCacheOptions cacheOptions) : IDataScopeProvider
{
    ...
    protected virtual async Task<DataScopeResult> ComputeAsync(long userId)
    {
        ...
        ILookup<long, long>? childrenByParent = null;   // 惰性加载:仅"本机构及以下"才需要整棵树
        ...
        foreach (var scope in scopes)
        {
            switch (scope.ScopeType)
            {
                ...
                case DataScopeType.OrgAndChildren:
                    ...
                    {
                        orgSet.Add(o2);
                        // 只取 Id/ParentId 两列:整棵树的全字段(名称、排序、备注、审计列)一列都用不上
                        childrenByParent ??= (await orgs.AsQueryable()
                                .Select(o => new { o.Id, o.ParentId })
                                .ToListAsync())
                            .ToLookup(o => o.ParentId, o => o.Id);
                        CollectDescendants(o2, childrenByParent, orgSet);
                    }
                    break;
    ...
    /// <summary>
    /// 把 root 的所有子孙机构 Id 收进 <paramref name="into"/>(广度优先,防御性去重避免坏数据成环死循环)。
    /// <para>按 ParentId 建好的 <see cref="ILookup{TKey,TElement}"/> 查子节点,避免每出队一个父节点就整表扫一遍——
    /// 机构数一多那样是平方级。</para>
    /// </summary>
    private static void CollectDescendants(long root, ILookup<long, long> childrenByParent, HashSet<long> into)
    {
        ...
        while (queue.Count > 0)
        {
            ...
            foreach (var childId in childrenByParent[parent])
                if (into.Add(childId)) queue.Enqueue(childId);   // Add 返回 false(已收过)即不再入队
        }
    }
