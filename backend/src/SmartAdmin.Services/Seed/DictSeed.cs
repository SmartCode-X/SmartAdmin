...
/// <summary>
/// 字典类型种子。播一个"通用状态"类型作为起步样例与冒烟锚点;固定 Id 保幂等,
/// 界面上对字典项的增删改不会被重启覆盖(种子只在缺失时补,不回改已存在行)。
/// </summary>
public class DictTypeSeed : ISeedData<SysDictType>
{
    ...
    /// <inheritdoc />
    public virtual IEnumerable<SysDictType> HasData() =>
    [
...
/// <summary>
/// 字典项种子。挂在 <see cref="DictTypeSeed.COMMON_STATUS_CODE"/> 下的两项;固定 Id 幂等。
/// </summary>
public class DictItemSeed : ISeedData<SysDictItem>
{
    /// <inheritdoc />
    public virtual IEnumerable<SysDictItem> HasData() =>
    [
