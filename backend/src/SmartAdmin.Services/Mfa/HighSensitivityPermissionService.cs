...
{
    /// <summary>高敏操作前置校验是否要求已绑定 TOTP:优先问 <see cref="IMfaPolicyService"/>,未注入则退回 Options 开关。</summary>
    protected virtual async Task<bool> IsTotpOnAsync()
    {
        if (mfaPolicy is not null) return await mfaPolicy.IsTotpFeatureEnabledAsync();
        return security?.IsTotpFeatureEnabled == true;
    }
    ...
    /// <summary>
    /// 校验操作者可执行高敏权限维护:账号须存在且启用;按策略开关要求已绑定 TOTP;
    /// 超管直接放行,否则要求持有 <paramref name="permissionCode"/> 对应权限码。
    /// </summary>
