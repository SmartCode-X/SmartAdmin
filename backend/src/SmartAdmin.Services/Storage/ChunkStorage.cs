...
/// <summary>
/// 分片上传临时存储。分片落本地临时区(存储根下的 <c>.chunks</c> 隐藏目录),
/// 按 uploadId 归组;合并时<b>单遍</b>算 SHA-256 供完整性校验,不额外多读一遍。
/// <para>ponytail: 分片临时区默认落本地磁盘——即便最终存储换 OSS/Minio,分片装配用本地临时也是常规做法;
/// 多副本要共享分片临时区就换掉 <see cref="IChunkStorage"/>。uploadId 派生自内容哈希(hex),
/// 这里再做一次字符白名单净化,兜底防路径穿越。</para>
/// </summary>
public class ChunkStorage(AdminUploadOptions options) : IChunkStorage
{
    ...
    public virtual async Task SaveChunkAsync(string uploadId, int index, Stream content, CancellationToken cancellationToken = default)
    {
    ...
    public virtual Task<IReadOnlyCollection<int>> GetReceivedIndexesAsync(string uploadId, CancellationToken cancellationToken = default)
    {
        var dir = PartsDir(uploadId);
    ...
    /// </summary>
    public virtual async Task<MergedChunks> MergeAsync(string uploadId, int chunkCount, CancellationToken cancellationToken = default)
    {
        var dir = PartsDir(uploadId);
    ...
    public virtual Task DiscardAsync(string uploadId, CancellationToken cancellationToken = default)
    {
        var dir = PartsDir(uploadId);
    ...
    /// </summary>
    public virtual Task<int> SweepStaleAsync(TimeSpan ttl, DateTimeOffset now, CancellationToken cancellationToken = default)
    {
