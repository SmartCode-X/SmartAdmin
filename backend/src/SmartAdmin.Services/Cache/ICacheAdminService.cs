...
/// <summary>
/// 缓存管理服务——面向管理员的<b>定向失效</b>动作。补自动失效(授权变更时 <c>RbacService</c>
/// 已自动 bump 门户代际 + 按用户清 perm/scope 键)覆盖不到的场景:有人<b>直接改库</b>致缓存陈旧的运维逃生舱。
/// <para>刻意<b>只清不看</b>:缓存值里有明文 OTP/验证码、键里内嵌手机号/IP(见 <see cref="SmartAdmin.Core.CacheKeys"/>),
...
public interface ICacheAdminService
{
    ...
    /// <summary>清全部系统配置缓存(按配置键,连同站点信息)。返回被清理的配置项数。</summary>
