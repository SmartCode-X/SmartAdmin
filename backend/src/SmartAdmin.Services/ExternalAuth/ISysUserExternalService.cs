...
/// <summary>未绑定外部身份的处理策略(可配置,默认拒绝)。</summary>
...
/// <summary>
/// 用户外部身份绑定服务。管 <c>sys_user_external</c> 表的增删查,并<b>收口</b>"运营配置"读取——
/// 启用开关 / 未绑定策略 / 按账号自动关联 / 开户默认角色·机构,统一走 <c>sys_config</c> 键
/// <c>sys.externalauth.{code}.{enabled|provisioning|linkByAccount|defaultRoleIds|defaultOrgId}</c>(运行时可改、无需重启)。
...
public interface ISysUserExternalService
{
    ...
    /// <summary>
    /// 未绑定的外部身份是否按账号自动关联(键 <c>...linkByAccount</c> = <c>true</c>,缺省关):
    /// 外部身份标识与某个本地账号完全相同时,首次登录直接绑到那个账号。
    /// 只该对「标识由企业统一分配、用户自己改不了」的 provider 打开,如企业微信的 userid。默认实现恒为关。
    /// </summary>
    Task<bool> IsLinkByAccountAsync(string provider) => Task.FromResult(false);
}
