...
/// <para>
/// 角色<b>定义</b>(新增/更新/删除/批量删除)是超管专属操作,越过路由权限之外的强约束——
/// 即便某普通管理员被授予了对应路由权限码,也不能创建/修改/删除角色本体(只能被超管授予"可转授"角色去分配给他人)。
/// <c>currentUser</c> 尾随可选:未注入(消费者精简子类/手工构造的实例)时视为可信系统上下文,不加限制。
/// </para>
/// </summary>
public class RoleService(IRepository<SysRole> roles, IRbacService rbac, ICurrentUser? currentUser = null) : IRoleService
{
    ...
    {
        EnsureSuperAdmin();   // 角色定义超管专属
        // 编码唯一(库有 idx_sys_role_code 唯一索引):查重纳入软删行,避免撞约束抛原生 500(同职位模块的成法)
        AdminException.ThrowIf(
    ...
    public virtual async Task DeleteAsync(long id)
    {
        ...
        // 软删不清关联:角色恢复后关联自动生效;真正清理在回收站 Purge。
    ...
    public virtual async Task DeleteBatchAsync(IReadOnlyCollection<long> ids)
    {
        ...
        await roles.Db.RunInTransactionAsync(async () =>
        {
