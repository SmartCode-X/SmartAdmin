...
/// 相对路径、同源、带合法签名(配了直链寿命时还带 <c>exp</c>);协议前缀(<c>http(s)://</c>)、多余查询参数、
/// 路径大小写偏差、伪造签名都直接拒绝(纵深防御)。
/// </summary>
