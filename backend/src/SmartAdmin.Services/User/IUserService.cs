...
/// <summary>
/// 用户管理服务——用户的增删改查分页 + 重置密码 + 启停用 + 角色分配。
/// <para>安全不变量(实现须守住):密码哈希绝不出接口;<c>IsSuperAdmin</c> 绝不经接口设置(防提权);
/// 超级管理员不可删除/停用(防自锁死)。类 public、方法 virtual 便于覆写。</para>
/// </summary>
public interface IUserService
{
    ...
    /// 不走分页 <c>MAX_SIZE=200</c> 截断;超过
    /// <c>SmartAdmin:Excel:MaxExportRows</c> 抛 <see cref="ErrorCode.ExportRowLimitExceeded"/>。
    /// </summary>
