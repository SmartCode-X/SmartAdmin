...
/// <summary>
/// 外部登录 / SSO 身份提供方。把"如何与某个 IdP 完成 OAuth2/OIDC 授权码交换"抽象成替换点:
/// 内核自带 <c>OidcExternalAuthProvider</c>(通吃 Keycloak/Entra/Authing/Auth0 等标准 OIDC);
/// 企业微信 / 钉钉等走可选包(<c>SmartAdmin.Auth.WeCom</c> / <c>.DingTalk</c>),消费者亦可实现本接口接自有 IdP。
...
/// <c>Microsoft.IdentityModel.*</c>,零新包);厂商 SDK 一律下沉可选包。</para>
/// </summary>
...
{
    /// <summary>
    /// 发起授权的浏览器 User-Agent(可空)。同一个 IdP 在不同客户端里要用不同的授权页时据此分流,
    /// 如企业微信客户端内置浏览器打不开扫码页、只能走网页授权;用不到的 provider 忽略即可。
    /// </summary>
