...
/// <summary>
/// 缓存提供者(扩展点)。用户权限码 / 菜单 / 字典 / 配置等热数据进缓存,变更时显式失效。
/// <para>默认实现 <c>MemoryCacheProvider</c>(进程内 <c>IMemoryCache</c>);
/// 装可选包 <c>SmartAdmin.Caching.Redis</c> 后前置注册 <c>RedisCacheProvider</c> 即整体替换,
/// 换成分布式缓存、多实例共享。</para>
...
public interface ICacheProvider
{
    ...
    /// <summary>
    /// 批量移除。授权面一改就是成百上千个用户的权限/范围键一起失效,逐键删在 Redis 上是逐个网络往返。
    /// <para>默认实现逐键调 <see cref="RemoveAsync"/>(进程内缓存本就没有往返,这样最省事);
    /// Redis 之类的 provider 应覆写成一次多键删除。</para>
    /// </summary>
    Task RemoveManyAsync(IEnumerable<string> keys, CancellationToken cancellationToken = default)
        => RemoveManyFallbackAsync(keys, cancellationToken);

    /// <summary>默认逐键移除兜底(供未覆写的实现复用)。</summary>
    protected async Task RemoveManyFallbackAsync(IEnumerable<string> keys, CancellationToken cancellationToken)
    {
