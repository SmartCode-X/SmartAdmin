...
/// <summary>
/// 邮件发送通道(与 <see cref="ISmsSender"/> 同款抽象)。内核只定义抽象、不带任何第三方邮件库
/// (运行时依赖纪律:仅 SqlSugarCore + Microsoft.*),默认实现仅把邮件写日志(开发期够用);配了 SMTP 主机则用
/// 内置 <c>SmtpEmailSender</c>(BCL <c>System.Net.Mail</c>)。
/// <para>接现代协议(OAuth2 SMTP / 云厂商 API):实现本接口并在 <c>AddSmartAdmin()</c> 之前注册即接管
/// (TryAdd 前置替换):<c>services.AddSingleton&lt;IEmailSender, MailKitEmailSender&gt;();</c></para>
