...
/// <param name="Svg">
/// 渲染后的 SVG 图(下发前端展示)。前端登录页直接以 <c>v-html</c> 注入 DOM,
...
/// <summary>
/// 验证码生成扩展点。内置三选:<c>char</c>(字符 SVG)、<c>path</c>(描边字形,明文不入标记、更抗爬)、
...
{
    /// <summary>本生成器的类型标识(如 <c>char</c>/<c>path</c>/<c>math</c>);配置按此选型。默认 <c>char</c>,自定义实现不覆写时即归入字符型。</summary>
    string Type => "char";
