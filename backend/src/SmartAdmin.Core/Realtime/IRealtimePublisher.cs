...
/// <summary>
/// 服务端→客户端实时推送通道。内核只定义抽象、不带任何长连接实现
/// (运行时依赖纪律:仅 SqlSugarCore + Microsoft.*);默认实现是<b>空操作</b>(<c>NoopRealtimePublisher</c>,
/// 关闭实时时业务代码照调不误)。AspNetCore 层在 <c>SmartAdmin:Realtime:Enabled</c> 开启时注册
...
/// <para>接自有实时通道(如 WebSocket 网关 / MQ 扇出):实现本接口并在 <c>AddSmartAdmin()</c> 之前注册即接管
/// (TryAdd 前置替换)。多副本要即时跨副本时,给内置 SignalR 叠 Redis backplane(消费者侧)。</para>
/// </summary>
