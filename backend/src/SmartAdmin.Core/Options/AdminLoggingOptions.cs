...
/// <summary>
/// 日志配置(对应 <c>SmartAdmin:Logging</c> 节)。
...
public class AdminLoggingOptions
{
    ...
    /// <summary>操作日志的落库方式与入参上限(见 <see cref="AdminOpLogOptions"/>)</summary>
    public AdminOpLogOptions OpLog { get; set; } = new();
}

/// <summary>
/// 操作日志配置(对应 <c>SmartAdmin:Logging:OpLog</c>)。
/// </summary>
public class AdminOpLogOptions
{
    /// <summary>
    /// 是否在请求线程里同步落库。<b>默认 false = 异步</b>:同步模式下,日志那条 INSERT 会挂在响应返回之前,
    /// 每个写请求都要多等它一次数据库往返。
    /// <para>置 true 用同步:审计要求"落库失败即请求失败"、或者容不得队列满时丢行的部署。</para>
    /// </summary>
    public bool Sync { get; set; }

    /// <summary>
    /// 异步队列容量,默认 10000。<b>满了丢最旧的</b>——审计里新的比旧的重要,
    /// 而阻塞请求线程去等一条日志是更坏的选择。逼近上限时会打告警,别把丢行咽下去。
    /// </summary>
    public int QueueCapacity { get; set; } = 10000;

    /// <summary>
    /// 入参 JSON 的字符上限,默认 8192。超了只留开头并标注原长度(仍是合法 JSON)。
    /// <para>务必设置上限:不截断的话,一次五千行的导入提交会把整份数据原样写进日志表。</para>
    /// </summary>
    public int ParamMaxChars { get; set; } = 8192;
}

/// <summary>
/// 文件日志配置(对应 <c>SmartAdmin:Logging:File</c> 节)。
