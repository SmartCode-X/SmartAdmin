...
/// <summary>
/// 头像 URL 校验(安全加固)——收窄"头像"字段可接受的取值,拦截存储型 XSS/开放重定向/内容注入面:
/// 头像字段本是自由文本(除软删标记外无任何约束),原样存入并原样吐给 <c>&lt;img src&gt;</c>,
/// 若允许任意外部地址或未签名的同源路径,等于放开一条可控 URL 注入通道。
/// <para>默认实现(<c>SmartAdmin.Services</c> 层)只放行 <c>null</c>/空白,或 <see cref="IFileUrlSigner"/> 签出的
