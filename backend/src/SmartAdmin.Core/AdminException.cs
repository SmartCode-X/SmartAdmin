...
public class AdminException : Exception
{
    ...
    public AdminException(ErrorCode code, IReadOnlyDictionary<string, object?>? args = null, string? fallbackMessage = null)
        : this(code, (Exception?)null, args, fallbackMessage)
    {
    }

    /// <summary>
    /// 同上,另附原始异常。用于"捕获外部依赖的异常 → 归一到一个业务码"的场合
    /// (对象存储 SDK、对端 HTTP、解密)。对外行为不变——信封里仍只有 code/msgKey/args;
    /// 原始那一层只进日志,否则「上传失败」到底是磁盘满了还是鉴权过期,事后查不出来。
    /// </summary>
    public AdminException(ErrorCode code, Exception? innerException,
        IReadOnlyDictionary<string, object?>? args = null, string? fallbackMessage = null)
        : base(fallbackMessage ?? code.GetMsgKey(), innerException)
    {
        Code = code;
    ...
    /// <summary>
    /// 便捷抛出:条件成立即抛。写法 <c>AdminException.ThrowIf(user is null, ErrorCode.UserNotFound);</c>
    /// <para><c>[DoesNotReturnIf(true)]</c> 让编译器知道调用返回时条件必为假:
    /// <c>ThrowIf(user is null || !user.Enabled, …)</c> 之后 <c>user</c> 已被推断为非空,不必再写 <c>user!</c>。</para>
    /// </summary>
    ...
    /// <summary>
    /// 同上,附一句兜底文案。用于校验结果既要回给前端(按 msgKey 翻译)、又要原样落库或回给非浏览器调用方的场合
    /// (设备接口、导入逐格校验的失败原因)——那时只有错误码不够,还得有一句人能读的话。
    /// </summary>
    public static void ThrowIf(
        [DoesNotReturnIf(true)] bool condition, ErrorCode code, string fallbackMessage,
        IReadOnlyDictionary<string, object?>? args = null)
    {
        if (condition) throw new AdminException(code, args, fallbackMessage);
    }
}
