...
/// <summary>
/// GitHub OAuth App 外部登录(可选包)。Code 硬固定 <c>github</c>;Subject = 用户数字 id;
/// scope 仅 <c>read:user</c>。构造注入 <see cref="HttpClient"/>,便于测试假 handler。
...
public class GitHubExternalAuthProvider : IExternalAuthProvider
{
    ...
    public const string HttpClientName = "SmartAdmin.Auth.GitHub";

    /// <summary>
    /// GitHub REST 要求有效 User-Agent;固定可识别产品名(无密钥/token)。
    /// 既挂在命名 HttpClient 默认头上,也在每条请求上再写一遍,避免仅依赖 DefaultRequestHeaders 的歧义。
    /// </summary>
    public const string UserAgentValue = "SmartAdmin-GitHubAuth";
    ...
    /// <summary>用授权码换取 GitHub access token。</summary>
    ...
    /// <summary>用 access token 拉取 GitHub 用户资料并映射为外部身份。</summary>
