...
/// <summary>
/// <see cref="ICacheProvider"/> 的 Redis 实现(可选包 <c>SmartAdmin.Caching.Redis</c>):
/// 用 StackExchange.Redis 把热数据放进共享 Redis,替代进程内 <c>MemoryCacheProvider</c>,支持多实例共享缓存。
/// <para>逻辑键统一追加 <see cref="AdminCacheOptions.KeyPrefix"/> 前缀(如 <c>smart:perm:123</c>);
...
public class RedisCacheProvider : ICacheProvider, IDisposable
{
    ...
    public RedisCacheProvider(AdminCacheOptions options)
    {
        if (string.IsNullOrWhiteSpace(options.RedisConnectionString))
            throw new InvalidOperationException("启用 Redis 缓存需配置 SmartAdmin:Cache:RedisConnectionString。");
        ...
        _mux = new Lazy<IConnectionMultiplexer>(() =>
        {
            ...
            // 显式 RequireTls:强制 SSL,避免连接串漏写 ssl=true 时明文连
            if (requireTls)
                config.Ssl = true;
    ...
    /// <inheritdoc />
    /// <remarks>
    /// 一次 <c>DEL</c> 删多个键,而不是逐个往返。分块 1000:Redis 是单线程的,一条命令删十万个键会把它按住不放,
    /// 期间所有客户端一起等——批量的意义是省往返,不是把阻塞攒成一坨。
    /// </remarks>
    public virtual async Task RemoveManyAsync(IEnumerable<string> keys, CancellationToken cancellationToken = default)
    {
        const int chunk = 1000;
        var batch = new List<RedisKey>(chunk);
        foreach (var key in keys)
        {
            batch.Add(Prefixed(key));
            if (batch.Count < chunk) continue;
