...
// ponytail: 与 BaseEntity 一样,概念上属于 Core,因携带 Sugar 特性而暂放本层;待 POCO 化再迁。
/// <summary>
/// 带机构数据范围的实体基类。业务表继承它即获得<b>数据范围锚点</b>
/// <see cref="CreateOrgId"/>,配合 <c>IDataScopeProvider</c> 全局过滤器(按 <see cref="IOrgScoped"/> 匹配),
/// 实现"本机构/本机构及以下/仅本人/自定义"隔离。
/// <para>不需要机构隔离的表(如全局字典、机构树自身)继续用 <see cref="BaseEntity"/>。</para>
/// <para><b>写路径守卫</b>:数据范围全局过滤器只作用于<b>查询(SELECT)</b>,不作用于按主键的
...
public abstract class DataEntity : BaseEntity, IOrgScoped
{
    ...
    /// 数据范围过滤器按它 + 当前用户的范围规则决定行可见性。为 null 表示不受机构范围约束(系统内建数据、或创建者无归属机构)。
    /// </summary>
...
/// <summary>
/// 带机构数据范围的<b>审计实体</b>:<see cref="AuditEntity"/> + <see cref="IOrgScoped"/>,<b>不含软删除</b>——
...
/// <para><b>写路径守卫</b>同 <see cref="DataEntity"/>:<see cref="SqlSugarRepository{TEntity}"/> 对 <see cref="IOrgScoped"/>
