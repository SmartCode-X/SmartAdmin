...
// ponytail: BaseEntity/PrimaryId 概念上属于 Core,因携带 SqlSugar 特性而暂留在本层;
// 待 Core 改用无特性 POCO + 外部映射时再迁移。
/// <summary>
/// 仅主键基类:只有雪花主键 <see cref="Id"/>,不带审计字段、不带软删除。
...
/// </list>
/// <para><b>与 <see cref="BaseEntity"/> 的取舍:</b>PrimaryId 更轻。种子 <c>ISeedData&lt;T&gt;</c> 支持它(明细表也能播种子),
/// 但内置泛型仓储 <c>IRepository&lt;T&gt;</c> 仍约束 <c>where T : AuditEntity</c> —— PrimaryId-only 实体通常经 <c>ISqlSugarClient</c>
/// 随主表在同一事务里读写。需要审计留痕/软删/被仓储托管的表,继续用 <see cref="BaseEntity"/>。</para>
/// </summary>
public abstract class PrimaryId
{
    /// <summary>主键(雪花 ID;种子数据可用固定小整数)。插入时为 0 则由审计 AOP 自动生成。</summary>
    [SugarColumn(IsPrimaryKey = true, ColumnDescription = "主键(雪花 ID;种子数据用固定小整数)")]
    public long Id { get; set; }
}

/// <summary>
/// 审计实体基类:主键 + 审计四件套,<b>不含软删除</b>。
...
{
    /// <summary>创建时间;插入时由审计 AOP 自动填充。</summary>
    [SugarColumn(ColumnDescription = "创建时间")]
    public DateTime CreateTime { get; set; }

    /// <summary>创建人用户 Id;插入时由审计 AOP 从当前登录用户填充,系统写入为 null。</summary>
    [SugarColumn(IsNullable = true, ColumnDescription = "创建人用户 Id")]
    public long? CreateUserId { get; set; }

    /// <summary>最后更新时间;每次更新由审计 AOP 自动刷新。</summary>
    [SugarColumn(IsNullable = true, ColumnDescription = "最后更新时间")]
    public DateTime? UpdateTime { get; set; }

    /// <summary>最后更新人用户 Id;每次更新由审计 AOP 从当前登录用户填充。</summary>
    [SugarColumn(IsNullable = true, ColumnDescription = "最后更新人用户 Id")]
    public long? UpdateUserId { get; set; }
}

/// <summary>
/// 实体基类:<see cref="AuditEntity"/>(主键 + 审计四件套)+ 软删除。
...
{
    /// <summary>软删除标记;<c>true</c> 即被全局查询过滤器排除,可经 <c>RestoreAsync</c> 恢复。</summary>
