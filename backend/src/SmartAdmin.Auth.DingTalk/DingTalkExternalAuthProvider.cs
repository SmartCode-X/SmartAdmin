...
/// <summary>
/// 钉钉外部登录 provider(可选包)。PC 端<b>扫码登录</b>(钉钉新版 OAuth2 授权码):构造授权 URL →
/// 用户扫码授权后带 <c>code</c> 回调 → 换用户 access_token → 拿 <c>unionId</c> 作为外部身份 <see cref="ExternalIdentity.Subject"/>。
/// <para><see cref="HttpClient"/> 由 DI 注入(与 GitHub/WeChat 做法一致)。方法 <c>virtual</c> 可覆写。</para>
/// </summary>
public class DingTalkExternalAuthProvider : IExternalAuthProvider
{
    ...
    public const string HttpClientName = "SmartAdmin.Auth.DingTalk";
