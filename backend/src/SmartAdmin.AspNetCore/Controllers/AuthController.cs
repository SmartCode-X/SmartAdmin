...
/// <summary>
/// 认证端点。标准 [ApiController]——可按模块禁用后由用户同路由接管。
/// Cookie 会话模式:refresh 进 HttpOnly Cookie,body 仅 accessToken;双提交 CSRF 由中间件校验。
/// 默认模式:body 下发 refreshToken。
/// </summary>
[ApiController]
[Route("api/v1/auth")]
public class AuthController(IAuthService auth, ICaptchaService captcha, AuthCookieService cookies) : ControllerBase
{
    ...
    /// Cookie 会话模式:body 可空,从 <c>smart_rt</c> Cookie 读取;成功后轮换 Cookie/CSRF。
    /// </summary>
    [HttpPost("refresh")]
    [AllowAnonymous]
    ...
    /// <summary>登出:吊销当前会话(sid 取自令牌)。仅需认证,不挂具体权限码。Cookie 会话模式同时清 Cookie。</summary>
    [HttpPost("logout")]
    [Authorize]
