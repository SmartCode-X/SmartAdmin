...
/// <summary>
/// 用户管理端点。全部 <c>[RolePermission]</c>——超管放行,普通用户需被授予对应路由权限码。
/// 安全细节(账号唯一、不出密码哈希、不建/改超管、超管不可删停)由 <see cref="IUserService"/> 保证。
/// <para>导入/导出:每个资源显式路由,不做通用 <c>/import/{code}</c> 控制器(权限模型要求)。
/// 三个 xlsx 端点返回 <see cref="FileStreamResult"/>,不进 <c>Result&lt;T&gt;</c> 信封。</para>
/// </summary>
[ApiController]
    ...
    AdminExcelOptions excel) : ControllerBase
{
    ...
    /// <c>[ActiveSession]</c> 而非 <c>[RolePermission]</c>;
    ...
    /// 必须 <c>[RolePermission]</c>——会调 <c>FindExistingKeysAsync</c> 查库判重,等于账号枚举面。
    /// </summary>
    ...
    /// <summary>下载错误报告 xlsx(原列 +「错误原因」列)。返回文件流,不进信封。</summary>
    ...
    /// <summary>按策略提交导入。<b>部分提交</b>:有错行跳过。服务端会重跑完整校验,不信任前端 Errors。</summary>
    ...
    /// GET 默认不进操作日志,显式挂 <c>[OperationLog]</c>。返回文件流,不进信封。
    /// </summary>
    [HttpGet("export")]
    [RolePermission]
    [OperationLog("导出用户")]
    ...
    /// <summary>校验导入文件非空、后缀 .xlsx、不超过 MaxImportFileSizeMb(复用 44xxx 码)。</summary>
