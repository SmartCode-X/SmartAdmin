...
public class SysFileController(IFileService files, IFileUrlSigner signer) : ControllerBase
{
    ...
    /// 与前端同源 → 存储型 XSS 偷令牌。因此按<b>后缀</b>解析安全媒体类型:是"可安全内联"的图片/PDF 才 inline,
    /// 其余一律 <c>application/octet-stream</c> + 强制另存(渲染不了就执行不了),再叠一层 <c>nosniff</c> 禁嗅探。</para>
    /// </summary>
    [HttpGet("{id}/view")]
    [AllowAnonymous]
    public async Task<IActionResult> View(long id, [FromQuery] string? sig, [FromQuery] long? exp)
    {
        // 校验先于查库:签名不对就不该让人拿它探测"这个 Id 存不存在"。exp 编在签名里,改了就验不过
        var expiresAt = exp is { } seconds ? DateTimeOffset.FromUnixTimeSeconds(seconds) : (DateTimeOffset?)null;
        if (!signer.Verify(id, sig, expiresAt)) return StatusCode(StatusCodes.Status403Forbidden);
