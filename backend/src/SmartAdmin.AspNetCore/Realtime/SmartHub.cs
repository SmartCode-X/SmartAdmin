...
/// <summary>
/// 实时推送 Hub。<b>纯服务端→客户端推送</b>,不定义任何客户端可调方法(推送即信号,负载让客户端回查)。
...
/// <c>access_token</c>(见 <c>SmartAdminSetup</c> 的 <c>OnMessageReceived</c>,仅在 Hub 路径采信)。</para>
/// <para>
/// JWT 通过期校验不代表会话仍活跃——强退/登出后原令牌在过期前仍是"合法令牌"。故连接建立时额外核验
...
/// </summary>
[Authorize]
public sealed class SmartHub(ISessionService sessions) : Hub
{
    /// <inheritdoc />
