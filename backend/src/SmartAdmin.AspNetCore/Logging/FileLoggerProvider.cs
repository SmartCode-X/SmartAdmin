...
/// 已用 <c>SmartAdmin:Id:WorkerId</c> 给文件名加 <c>-w1</c> 后缀兜住(多副本本来就强制配唯一机器号,不新造概念);
...
public sealed class FileLoggerProvider : ILoggerProvider
{
    ...
    private async Task ConsumeAsync()
    {
        ...
        catch (Exception ex)
        {
            ...
            Console.Error.WriteLine($"[SmartAdmin] 文件日志消费循环异常终止,后续日志不再写盘: {ex}");
        }
        finally
        {
    ...
    /// <para><b>永远 append,绝不截断。</b>有些同类实现换文件时走 <c>append: false</c> → <c>SetLength(0)</c>,
    ...
    private sealed class LevelWriter(string dir, string suffix, AdminFileLogOptions options, TimeProvider time) : IDisposable
    {
        ...
        private void Fail(string what, Exception ex, bool fatal)
        {
            ...
            Console.Error.WriteLine($"[SmartAdmin] {what}: {ex.Message} {tail}");
        }
    }
}
