...
/// <summary>
/// 操作日志过滤器——全局挂载,<b>默认记录一切写操作</b>(安全默认:审计不能靠人记得加特性)。
/// 在动作执行前快照脱敏入参与起始时刻,执行后落一条操作日志(耗时 + 业务码 + 操作人/IP/UA)。
/// <para>耗时用 <see cref="TimeProvider.GetTimestamp"/> 测(单调时钟,不受系统时间回拨影响)。</para>
/// <para>写操作默认全部记录(opt-out,而非按 <see cref="OperationLogAttribute"/> 逐个 opt-in):
/// 角色授权/数据范围/系统配置/停用用户/强制下线这些<b>最需要审计的动作</b>不能因漏挂特性而无痕,
/// 消费方自建的控制器同样默认留痕。只放行两类不记:读操作与匿名端点,判定逻辑见 <see cref="ShouldLog"/>。</para>
/// </summary>
internal sealed class OperationLogFilter(
    ILogService logService,
    TimeProvider timeProvider,
    AdminLoggingOptions logging) : IAsyncActionFilter
{
