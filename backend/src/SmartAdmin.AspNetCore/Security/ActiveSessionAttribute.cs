...
/// <summary>
/// 会话活性校验(强退即时生效)——认证 + 会话仍活跃,但<b>不要求具体权限码</b>。
/// <para>用于个人中心/登出这类"任何已登录用户可用"的端点:仅 <c>[Authorize]</c> 只校验 JWT 签名与有效期,
/// 会话被强退/登出后仍能被未过期的 access token 继续调用(最长到令牌自然过期);挂本特性后与
/// <see cref="RolePermissionAttribute"/> 同源每请求校验 <see cref="ISessionService.IsActiveAsync"/>,强退即 401。</para>
...
public class ActiveSessionAttribute : Attribute, IAsyncAuthorizationFilter
{
    /// <inheritdoc />
    public virtual async Task OnAuthorizationAsync(AuthorizationFilterContext context)
    {
        ...
        // 经 API Key 认证的机器主体没有会话:认证即放行(与 [RolePermission] 同一条规则)
        if (!user.HasClaim(c => c.Type == TokenClaimNames.API_KEY))
        {
    ...
    /// <summary>401 出口(统一信封 40006),与 <see cref="RolePermissionAttribute"/> 一致。</summary>
