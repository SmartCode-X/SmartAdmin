...
internal static class JwtKeyResolver
{
    ...
    public static SymmetricSecurityKey Resolve(AdminJwtOptions options, IHostEnvironment env, ILogger logger)
    {
        ...
        if (!string.IsNullOrEmpty(options.SecretKey))
        {
            ...
            if (bytes.Length < MIN_KEY_BYTES)
                throw new InvalidOperationException(
                    $"SmartAdmin:Jwt:SecretKey 过短({bytes.Length} 字节 < {MIN_KEY_BYTES} 字节)。" +
                    "弱密钥可被暴力破解进而伪造任意用户令牌,请配置至少 32 字节的随机串。");
            return new SymmetricSecurityKey(bytes);
        }

        // 模式二:未配置 + 生产 → fail-fast(生产绝不允许开发密钥)
        if (!env.IsDevelopment())
            throw new InvalidOperationException(
                "生产环境必须显式配置 SmartAdmin:Jwt:SecretKey(至少 32 字节随机串);" +
                "禁止使用自动生成的开发密钥——否则多副本密钥不一致会随机 401,密钥泄露可伪造任意用户令牌。");
        ...
        logger.LogWarning(
            "SmartAdmin: 未配置 SmartAdmin:Jwt:SecretKey,正在使用开发密钥({Path})。" +
