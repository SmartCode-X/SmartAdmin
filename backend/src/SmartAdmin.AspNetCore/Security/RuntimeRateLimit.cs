...
/// <summary>
/// 限流参数的运行时快照。限流中间件每请求都要读阈值,而读 <c>SysConfig</c> 是异步查库,不能挂在那条路径上;
/// 故用一个单例快照:启动时从 <c>SysConfig</c>(缺失/解析失败回退 Options)载入,订阅 <see cref="ConfigChangedEvent"/>
/// 即时刷新,另有定期重载让别的副本改的配置也能收敛过来。中间件同步读 <see cref="Current"/>。
/// <para><c>TryAddSingleton</c> 注册且非密封:消费方前置注册子类(覆写 <see cref="RefreshAsync"/> 从别处取阈值)即接管。</para>
/// </summary>
public class RuntimeRateLimit(
...
{
    /// <summary>
    /// 一次原子读到的一整组限流参数(引用整体替换,选择器不会读到撕裂的中间态)。
    /// <paramref name="KeyPermitPerWindow"/> 是机器端按 API Key 计数的阈值,只来自 Options(不走配置中心);缺省 0 = 不限。
    /// </summary>
    public sealed record Snapshot(bool Enabled, int WindowSeconds, int PermitPerWindow, int AuthPermitPerWindow, int KeyPermitPerWindow = 0);

    private volatile Snapshot? _current;
    private IDisposable? _sub;

    /// <summary>当前生效快照;尚未载入时回退 Options 默认。</summary>
    public virtual Snapshot Current => _current ?? Fallback();

    /// <summary>尚未从 DB 载入快照前的回退值,直接读 Options 配置。</summary>
    protected virtual Snapshot Fallback()
    {
        var rl = security.RateLimit;
        return new(rl.Enabled, rl.WindowSeconds, rl.PermitPerWindow, rl.AuthPermitPerWindow, rl.KeyPermitPerWindow);
    }
    ...
    public virtual async Task RefreshAsync()
    {
        try
        {
            ...
            _current = new Snapshot(rl.Enabled && dbEnabled, window, permit, authPermit, rl.KeyPermitPerWindow);
        }
        catch (Exception ex)
        {
