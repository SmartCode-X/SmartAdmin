...
/// <summary>
/// 统一返回兜底过滤器。<b>便利/兜底</b>用途:用户业务控制器直接 <c>return dto;</c>
...
/// <para><b>契约提示</b>:本过滤器在结果执行阶段包信封,对 ApiExplorer 不可见——故裸返回 <c>dto</c> 的
/// 端点,其 OpenAPI 200 schema 记为 <c>dto</c>(无信封外壳),而运行时实为 <c>Result&lt;dto&gt;</c>。若该端点要经
/// <c>openapi-typescript</c> 生成前端类型,<b>请显式声明返回 <c>Result&lt;T&gt;</c></b>(与内置控制器一致),
/// 使契约与运行时对齐;否则前端会把 data 误当成顶层 dto。全量裸返回自动改写 schema 的方案留待后续版本。</para>
/// </summary>
// public(非 internal):判定逻辑 <see cref="TryWrap"/> 是纯函数、便于单测;用户也可能要调整它在过滤器链里的次序。
public class ResultEnvelopeFilter : IAsyncResultFilter
{
    /// <inheritdoc />
    public virtual Task OnResultExecutionAsync(ResultExecutingContext context, ResultExecutionDelegate next)
    {
        // [SkipEnvelope]:对接第三方 / 设备端要固定响应形状的端点,成功返回原样放出
        if (context.ActionDescriptor.EndpointMetadata.OfType<SkipEnvelopeAttribute>().Any()) return next();
